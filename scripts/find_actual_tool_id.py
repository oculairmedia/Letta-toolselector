...
headers = {"Authorization": f"Bearer {LETTA_API_KEY}", "Content-Type": "application/json", "Accept": "application/json"}
...
if response.status_code == 200:
    ...
    for tool in tools:
        ...
        if "find_tools" in tool_name.lower() or "find tools" in tool_name.lower():
            ...
            print()
        elif "tool" in tool_name.lower() and (
            "find" in tool_name.lower() or "search" in tool_name.lower() or "attach" in tool_name.lower()
        ):
            find_tools_candidates.append(tool)
