...
def test_comprehensive_search():
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        try:
            ...
            print(f"Error: {str(e)}")
            results_summary.append({"query": query, "error": str(e)})
    ...
    for result in results_summary:
        if "error" in result:
            ...
            print(
                f"{result['query'][:30]:<30} | {result['results_count']:<8} | {result['top_match'][:30]:<30} | {result['top_score']:<8.2f} | {result['search_time']:<8.2f}"
            )
