...
def main():
    ...
    success, data = test_api_endpoint(
        "/api/v1/cost-control/status", description="Get overall cost control system status"
    )
    if success and data:
    ...
    success, data = test_api_endpoint("/api/v1/cost-control/budget", description="Get all budget limits and status")
    if success and data:
        budgets = data.get("data", {})
        print(f"   🎯 Active Budget Limits: {len(budgets)}")
        for key, budget in list(budgets.items())[:3]:  # Show first 3
            print(
                f"      {key}: ${budget['current_cost']:.4f}/${budget['limit']:.2f} ({budget['percentage_used']:.1f}%)"
            )

    print()

    # Test 3: Get cost summary for daily period
    success, data = test_api_endpoint("/api/v1/cost-control/summary?period=daily", description="Get daily cost summary")
    if success and data:
    ...
    success, data = test_api_endpoint(
        ...
        method="POST",
        data={"operation_type": "openai_embedding", "params": {"token_count": 1000}},
    ...
    success, data = test_api_endpoint("/api/v1/cost-control/alerts?hours=1", description="Get alerts from last hour")
    if success and data:
