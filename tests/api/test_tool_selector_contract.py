...
class TestSearchAPIContract:
    ...
    def test_search_returns_200_with_valid_query(self, client):
        ...
        response = client.post("/api/v1/tools/search", json={"query": "create issue", "limit": 5})
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"

    def test_search_response_is_list(self, client):
        """Search response is a list of tools (not wrapped in object)."""
        response = client.post("/api/v1/tools/search", json={"query": "file operations", "limit": 3})
    ...
    def test_search_tools_have_required_fields(self, client, tool_schema):
        ...
        response = client.post("/api/v1/tools/search", json={"query": "send message", "limit": 5})
        assert response.status_code == 200
        tools = response.json()
    ...
    def test_search_tools_have_scores(self, client):
        ...
        response = client.post("/api/v1/tools/search", json={"query": "read file", "limit": 3})
        assert response.status_code == 200
        tools = response.json()
    ...
    def test_search_respects_limit(self, client):
        ...
        limit = 3
        response = client.post("/api/v1/tools/search", json={"query": "tool", "limit": limit})
        assert response.status_code == 200
        tools = response.json()
    ...
    def test_search_with_empty_query_returns_error(self, client):
        ...
        response = client.post("/api/v1/tools/search", json={"query": "", "limit": 5})
...
class TestAttachAPIContract:
    ...
    def test_attach_requires_agent_id(self, client):
        ...
        response = client.post("/api/v1/tools/attach", json={"tool_ids": ["some-tool-id"]})
        assert response.status_code in [400, 422], f"Expected 400/422, got {response.status_code}"

    def test_attach_requires_tool_specification(self, client):
        """Attach without tool_ids or query returns error."""
        response = client.post("/api/v1/tools/attach", json={"agent_id": "test-agent-id"})
    ...
    def test_attach_with_invalid_agent_id_handled(self, client):
        ...
        response = client.post(
            "/api/v1/tools/attach", json={"agent_id": "non-existent-agent-12345", "tool_ids": ["some-tool-id"]}
        )
