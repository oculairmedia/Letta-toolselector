...
class ABTestingFramework:
        ...
        try:
            ...
            if config.sample_size_per_group is None:
                config.sample_size_per_group = self._calculate_sample_size(
                    config.minimum_detectable_effect, config.alpha, config.beta
                )
    ...
    async def start_experiment(self, experiment_id: str, dataset_id: str, metadata: Dict[str, Any] = None) -> str:
    ...
    async def _execute_experiment(self, execution: ExperimentExecution):
        ...
        try:
            ...
            self.logger.info(
                f"Executing A/B test {execution.id}: {config.control_config_id} vs {config.treatment_config_id}"
            )
    ...
    async def _monitor_experiment_progress(self, execution: ExperimentExecution):
        ...
        while execution.status == ExperimentStatus.RUNNING:
            ...
            if control_job.status.value in ["completed", "failed"] and treatment_job.status.value in [
                "completed",
                "failed",
            ]:
                break
    ...
    async def _perform_final_analysis(self, execution: ExperimentExecution) -> ExperimentResult:
        ...
        return await self._calculate_experiment_result(execution, control_job.results, treatment_job.results, 1.0)
    ...
    async def _calculate_experiment_result(
        ...
        control_metrics, _ = self.metrics_engine.calculate_batch_metrics(queries, control_results, metric_config)
        treatment_metrics, _ = self.metrics_engine.calculate_batch_metrics(queries, treatment_results, metric_config)
        ...
        ci_lower, ci_upper = self._calculate_confidence_interval(primary_control, primary_treatment, config.alpha)
    ...
    def _perform_statistical_test(
        ...
        try:
            ...
            return {"statistic": statistic, "p_value": p_value, "method": method, "degrees_of_freedom": df}

        except Exception as e:
            return {"statistic": 0.0, "p_value": 1.0, "method": f"Failed: {str(e)}", "degrees_of_freedom": None}

    def _calculate_confidence_interval(
        self, control_values: List[float], treatment_values: List[float], alpha: float
    ) -> Tuple[float, float]:
    ...
    def _calculate_statistical_power(self, effect_size: float, n_control: int, n_treatment: int, alpha: float) -> float:
    ...
    def _should_stop_early(self, result: ExperimentResult, config: ExperimentConfiguration) -> bool:
        ...
        if (
            ...
            and result.relative_improvement > config.minimum_detectable_effect
            and result.statistical_power > 0.9
        ):
            return True
