...
class TestPreAttachPruning:
    ...
    async def _get_tool_counts(self, agent_id: str) -> Dict[str, int]:
        ...
        mcp_count = sum(
            ...
            or (not _is_letta_core_tool(tool) and tool.get("tool_type") == "custom")
        )
        core_count = len(tools) - mcp_count

        return {"total": len(tools), "mcp": mcp_count, "core": core_count}

    async def _attach_tools_via_api(
        self, api_client, api_base_url: str, agent_id: str, query: str, limit: int = 10, auto_prune: bool = False
    ) -> Dict[str, Any]:
        """Helper to call the attach tools API endpoint."""
        url = f"{api_base_url}/api/v1/tools/attach"
        payload = {"agent_id": agent_id, "query": query, "limit": limit, "auto_prune": auto_prune}
    ...
    async def test_preattach_pruning_enforces_max_total_tools(self, api_client, test_agent_id, api_base_url):
        ...
        try:
            ...
            result = await self._attach_tools_via_api(
                api_client, api_base_url, test_agent_id, query="data processing and analysis tools", limit=5
            )
    ...
    async def test_preattach_pruning_enforces_max_mcp_tools(self, api_client, test_agent_id, api_base_url):
        ...
        try:
            ...
            result = await self._attach_tools_via_api(
                api_client, api_base_url, test_agent_id, query="external API integration tools", limit=3
            )
    ...
    async def test_preattach_pruning_respects_min_mcp_tools(self, api_client, test_agent_id, api_base_url):
        ...
        try:
            ...
            # Attach tools
            result = await self._attach_tools_via_api(
                api_client, api_base_url, test_agent_id, query="utility and helper tools", limit=3
            )
    ...
    async def test_preattach_pruning_skipped_when_no_query(self, api_client, test_agent_id, api_base_url):
        ...
        try:
            ...
            assert "error" in result or "success" in result, "API should return structured response even without query"
