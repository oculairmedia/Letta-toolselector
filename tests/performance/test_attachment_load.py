...
async def make_search_request(session: aiohttp.ClientSession, query: str, limit: int = 5) -> RequestResult:
    """Make a single search request and measure latency."""
    start = time.perf_counter()
    try:
        async with session.post(
            SEARCH_ENDPOINT, json={"query": query, "limit": limit}, timeout=aiohttp.ClientTimeout(total=30)
        ) as response:
            await response.json()
            duration_ms = (time.perf_counter() - start) * 1000
            return RequestResult(duration_ms=duration_ms, success=response.status == 200, status_code=response.status)
    except asyncio.TimeoutError:
        duration_ms = (time.perf_counter() - start) * 1000
        return RequestResult(duration_ms=duration_ms, success=False, status_code=0, error="Timeout")
    except Exception as e:
        duration_ms = (time.perf_counter() - start) * 1000
        return RequestResult(duration_ms=duration_ms, success=False, status_code=0, error=str(e))


async def run_concurrent_requests(num_requests: int, queries: List[str]) -> List[RequestResult]:
