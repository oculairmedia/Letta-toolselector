...
class RerankingPerformanceValidator:
    ...
    async def search_with_reranking(self, query: str, limit: int = 10) -> Tuple[List[SearchResult], float]:
        ...
        payload = {
            "query": query,
            "limit": limit,
            "reranker_config": {"enabled": True, "model": "dengcao/Qwen3-Reranker-4B:Q5_K_M", "provider": "ollama"},
        }
    ...
    async def search_without_reranking(self, query: str, limit: int = 10) -> Tuple[List[SearchResult], float]:
        ...
        async with self.session.post(
            f"{self.api_base_url}/api/v1/tools/search", json=payload, headers={"Content-Type": "application/json"}
        ) as response:
            response_time = (time.time() - start_time) * 1000
            ...
            if response.status == 200:
                ...
                if isinstance(data, list):
                    ...
                    for i, item in enumerate(data[:limit]):
                        results.append(SearchResult(name=item["name"], score=item.get("score", 0), rank=i + 1))
                    return results, response_time
        ...
        for i, test_case in enumerate(test_queries, 1):
            ...
            try:
                ...
                precision_improvement = (
                    (rerank_metrics.precision_at_5 - regular_metrics.precision_at_5)
                    / max(regular_metrics.precision_at_5, 0.001)
                ) * 100
    ...
    def calculate_aggregate_results(
        self, rerank_metrics: List[PerformanceMetrics], regular_metrics: List[PerformanceMetrics]
    ) -> Dict[str, Any]:
        ...
        precision_improvement = (
            (avg_rerank_precision - avg_regular_precision) / max(avg_regular_precision, 0.001)
        ) * 100
