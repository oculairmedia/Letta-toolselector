...
def test_adapter_reranking():
    ...
    try:
        ...
        response = requests.post(f"{adapter_url}/rerank", json=test_request, timeout=30)
...
def test_weaviate_with_reranking():
    ...
    try:
        from weaviate_tool_search_with_reranking import search_tools_with_reranking, test_reranking_capability
        ...
        test_queries = ["create blog posts", "manage github repositories", "send email notifications"]
        ...
        for query in test_queries:
            ...
            results_with = search_tools_with_reranking(query=query, limit=3, use_reranking=True)

            # Search without reranking (for comparison)
            results_without = search_tools_with_reranking(query=query, limit=3, use_reranking=False)
...
def test_performance():
    ...
    for size in test_sizes:
        test_request = {
            ...
            "documents": [
                f"Tool{i} - A tool for various data operations and analysis tasks" for i in range(1, size + 1)
            ],
        }

        try:
            start_time = time.time()
            response = requests.post(f"{adapter_url}/rerank", json=test_request, timeout=60)
            elapsed = time.time() - start_time
