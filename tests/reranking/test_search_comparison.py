...
def search_tools(query, limit=5, enable_reranking=False):
    ...
    if enable_reranking:
        ...
        payload = {
            "query": query,
            "limit": limit,
            "reranker_config": {"enabled": True, "model": "bge-reranker-v2-m3", "base_url": "http://localhost:8091"},
        }
    else:
...
queries = ["create blog post", "file operations", "memory management", "huly project issues", "agent configuration"]
