...
def test_attach_and_trigger_prune():
    ...
    if not AGENT_ID or AGENT_ID == "your_agent_id_here":
        logger.error(
            "AGENT_ID is not set or is set to the placeholder. Please set TEST_AGENT_ID in your .env file or directly in the script."
        )
        return

    if (
        not TOOL_SERVER_URL or TOOL_SERVER_URL == "http://localhost:5001"
    ):  # Check against a common default if .env is not specific
        logger.warning(
            f"TOOL_SERVER_URL is using a default or common value: {TOOL_SERVER_URL}. Ensure this is correct for your local setup."
        )
        ...
        if response.status_code == 200 and response_data.get("success"):
            ...
            if successful_attachments:
                ...
                for tool in successful_attachments:
                    logger.info(
                        f"  - ID: {tool.get('tool_id')}, Name: {tool.get('name')}, Score: {tool.get('match_score')}"
                    )

            logger.info(
                "Check the logs of your api_server.py to see details of the attachment and subsequent pruning process."
            )
        else:
