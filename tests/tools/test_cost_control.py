...
async def test_basic_functionality():
    ...
    await manager.record_cost(CostCategory.VECTOR_DATABASE, "test_weaviate_query", 0.01, {"query_count": 10})

    await manager.record_cost(CostCategory.LETTA_API, "test_agent_call", 0.02, {"call_count": 2})
...
async def test_period_calculations():
    ...
    for period in periods:
        ...
        duration = end - start
        print(
            f"   📅 {period.value}: {start.strftime('%Y-%m-%d %H:%M')} → {end.strftime('%Y-%m-%d %H:%M')} ({duration})"
        )
