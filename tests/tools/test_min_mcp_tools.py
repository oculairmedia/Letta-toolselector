...
headers = {"Authorization": f"Bearer {LETTA_PASSWORD}", "Content-Type": "application/json"}


def get_agent_tools(agent_id):
    """Get all tools attached to an agent."""
    response = requests.get(f"{LETTA_API_URL}/agents/{agent_id}/tools", headers=headers)
...
def test_min_mcp_tools_enforcement():
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE_URL}/api/v1/tools/attach", json=attach_payload, headers={"Content-Type": "application/json"}
        )
