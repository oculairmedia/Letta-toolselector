...
def test_never_detach_config():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "api_server", os.path.join(os.path.dirname(__file__), "tool-selector-api", "api_server.py")
        )
        api_server = importlib.util.module_from_spec(spec)

        # Mock the required dependencies that might not be available
        sys.modules["quart"] = type(
            "MockQuart", (), {"Quart": lambda name: None, "request": None, "jsonify": lambda x: x}
        )()
        sys.modules["weaviate_tool_search"] = type(
            "MockWeaviate",
            (),
            {
        ...
        sys.modules["upload_tools_to_weaviate"] = type("MockUpload", (), {"upload_tools": lambda: None})()
...
def test_tool_categorization_logic():
    ...
    for tool in mock_tools:
        ...
        is_never_detach_tool = (
            ...
            or any(never_detach_name.lower() in tool_name for never_detach_name in never_detach_tools)
        )
