...
def test_prune_with_different_prompts():
    ...
    for test_case in test_cases:
        ...
        try:
            response = requests.post(prune_url, json=payload, headers={"Content-Type": "application/json"}, timeout=30)
            ...
            if response.status_code == 200:
                ...
                results.append({"test": test_case["name"], "embedding_success": False, "error": response.text})

        except Exception as e:
            print(f"❌ Exception: {e}")
            results.append({"test": test_case["name"], "embedding_success": False, "error": str(e)})
...
def test_search_endpoint():
    ...
    test_queries = ["job search", "memory management", "integration setup", "conversation search"]
    ...
    for query in test_queries:
        ...
        try:
            ...
            response = requests.post(search_url, json=payload, headers={"Content-Type": "application/json"}, timeout=15)
