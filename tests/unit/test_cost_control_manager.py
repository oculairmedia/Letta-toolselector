...
class TestCostEntry:
    ...
    def test_cost_entry_defaults(self):
        ...
        entry = CostEntry(timestamp=datetime.now(), category=CostCategory.LETTA_API, operation="test_op", cost=1.0)
...
class TestBudgetLimit:
    ...
    def test_budget_limit_overall(self):
        ...
        budget = BudgetLimit(category=None, period=BudgetPeriod.MONTHLY, limit=100.0)
...
class TestCostSummary:
    ...
    def test_cost_summary_creation(self):
        ...
        summary = CostSummary(
            ...
            total_cost=15.75,
            cost_by_category={CostCategory.EMBEDDING_API: 10.0, CostCategory.LETTA_API: 5.75},
            entry_count=25,
        )
...
class TestCostControlManager:
    ...
    async def test_record_cost(self, cost_manager):
        ...
        allowed = await cost_manager.record_cost(
            CostCategory.EMBEDDING_API, "test_operation", 0.05, {"test": "metadata"}
        )
    ...
    async def test_record_cost_persists(self, cost_manager):
        ...
        await cost_manager.record_cost(CostCategory.LETTA_API, "test_operation", 1.25)
    ...
    async def test_set_budget_limit(self, cost_manager):
        ...
        key = await cost_manager.set_budget_limit(CostCategory.EMBEDDING_API, BudgetPeriod.DAILY, 5.0, hard_limit=True)
    ...
    async def test_remove_budget_limit(self, cost_manager):
        ...
        # Set a budget
        await cost_manager.set_budget_limit(CostCategory.EMBEDDING_API, BudgetPeriod.DAILY, 5.0)

        # Remove it
        removed = await cost_manager.remove_budget_limit(CostCategory.EMBEDDING_API, BudgetPeriod.DAILY)
    ...
    async def test_estimate_operation_cost(self, cost_manager):
        ...
        # Test embedding cost
        cost1 = await cost_manager.estimate_operation_cost("openai_embedding", token_count=1000)
        assert cost1 > 0

        # Test Weaviate query cost
        cost2 = await cost_manager.estimate_operation_cost("weaviate_query", query_count=5)
        assert cost2 > 0

        # Test Weaviate insert cost
        cost3 = await cost_manager.estimate_operation_cost("weaviate_insert", insert_count=10)
        assert cost3 > 0

        # Test Letta API cost
        cost4 = await cost_manager.estimate_operation_cost("letta_api_call", call_count=3)
        assert cost4 > 0
    ...
    async def test_calculate_costs_for_period(self, cost_manager):
        ...
        total = cost_manager._calculate_costs_for_period(CostCategory.EMBEDDING_API, period_start, period_end)

        assert total == 3.0  # 1.0 + 2.0

        # Calculate for all categories
        total_all = cost_manager._calculate_costs_for_period(None, period_start, period_end)
    ...
    async def test_budget_violation_generates_alert(self, cost_manager):
        ...
        await cost_manager.set_budget_limit(CostCategory.EMBEDDING_API, BudgetPeriod.DAILY, 1.0, hard_limit=False)

        # Record a cost that exceeds the budget
        allowed = await cost_manager.record_cost(CostCategory.EMBEDDING_API, "expensive_operation", 2.0)
    ...
    async def test_hard_limit_blocks_operation(self, cost_manager):
        ...
        await cost_manager.set_budget_limit(CostCategory.EMBEDDING_API, BudgetPeriod.DAILY, 1.0, hard_limit=True)
    ...
    async def test_get_budget_status(self, cost_manager):
        ...
        # Set a budget limit
        await cost_manager.set_budget_limit(CostCategory.EMBEDDING_API, BudgetPeriod.DAILY, 10.0)

        # Record some costs
        await cost_manager.record_cost(CostCategory.EMBEDDING_API, "op1", 3.0)
    ...
    async def test_reset_period_costs(self, cost_manager):
        ...
        await cost_manager.reset_period_costs(CostCategory.EMBEDDING_API, BudgetPeriod.DAILY)
    ...
    async def test_add_alert_handler(self, cost_manager):
        ...
        await cost_manager.set_budget_limit(CostCategory.EMBEDDING_API, BudgetPeriod.DAILY, 1.0)

        # Trigger alert
        await cost_manager.record_cost(CostCategory.EMBEDDING_API, "op", 2.0)
    ...
    async def test_get_recent_alerts(self, cost_manager):
        ...
        cost_manager.alerts_cache = [
            ...
            CostAlert(now - timedelta(days=2), AlertLevel.INFO, CostCategory.LETTA_API, "Old alert", 2.0, 10.0, 20.0),
        ]
    ...
    async def test_track_operation_context_manager(self, cost_manager):
        ...
        async with cost_manager.track_operation(CostCategory.EMBEDDING_API, "test_operation", estimated_cost=0.01):
            # Operation code here
            pass
    ...
    async def test_track_operation_blocks_on_budget(self, cost_manager):
        ...
        with pytest.raises(ValueError, match="Operation blocked"):
            async with cost_manager.track_operation(CostCategory.EMBEDDING_API, "expensive_op", estimated_cost=2.0):
                pass
...
class TestEdgeCases:
    ...
    async def test_disabled_budget_limit(self, cost_manager):
        ...
        await cost_manager.set_budget_limit(
            CostCategory.EMBEDDING_API, BudgetPeriod.DAILY, 0.01, hard_limit=True, enabled=False
        )

        # Record a cost that would exceed the disabled budget
        allowed = await cost_manager.record_cost(CostCategory.EMBEDDING_API, "test_op", 10.0)
    ...
    async def test_zero_budget_limit(self, cost_manager):
        ...
        await cost_manager.set_budget_limit(CostCategory.EMBEDDING_API, BudgetPeriod.DAILY, 0.0)
    ...
    async def test_alert_threshold_deduplication(self, cost_manager):
        ...
        await cost_manager.set_budget_limit(
            CostCategory.EMBEDDING_API, BudgetPeriod.DAILY, 10.0, alert_thresholds=[0.5, 0.8, 0.95]
        )

        # Record cost to trigger 50% threshold
        await cost_manager.record_cost(CostCategory.EMBEDDING_API, "op1", 5.0)

        alerts_count_1 = len(cost_manager.alerts_cache)

        # Record another small cost (still in same threshold range)
        await cost_manager.record_cost(CostCategory.EMBEDDING_API, "op2", 0.1)
