...
def mock_openai_response(sample_embeddings):
    ...
    mock_response = Mock()
    mock_response.data = [Mock(embedding=sample_embeddings[0]), Mock(embedding=sample_embeddings[1])]
...
class TestOpenAIProvider:
    ...
    async def test_openai_provider_initialization(self):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            provider = OpenAIEmbeddingProvider(model="text-embedding-3-small", dimensions=1536, api_key="test-key")
    ...
    async def test_openai_get_embeddings(self, mock_openai_response, sample_embeddings):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            provider = OpenAIEmbeddingProvider(model="text-embedding-3-small", dimensions=4, api_key="test-key")
...
class TestOllamaProvider:
    ...
    def test_ollama_provider_initialization(self):
        ...
        provider = OllamaEmbeddingProvider(model="test-model", dimensions=2560, base_url="http://localhost:11434")
    ...
    def test_ollama_base_url_stripped(self):
        ...
        provider = OllamaEmbeddingProvider(model="test", dimensions=100, base_url="http://localhost:11434/")
    ...
    async def test_ollama_check_model_availability(self):
        ...
        provider = OllamaEmbeddingProvider(model="test-model", dimensions=100, base_url="http://localhost:11434")

        # Mock the response with proper async context manager
        mock_response = Mock()
        mock_response.status = 200
        mock_response.json = AsyncMock(return_value={"models": [{"name": "test-model"}, {"name": "other-model"}]})
    ...
    async def test_ollama_get_embeddings(self, mock_ollama_response, sample_embeddings):
        ...
        provider = OllamaEmbeddingProvider(model="test-model", dimensions=4, base_url="http://localhost:11434")
...
class TestQwen3Provider:
    ...
    def test_qwen3_build_extra_body(self):
        ...
        provider = Qwen3EmbeddingProvider(
            model="qwen3", dimensions=2560, pooling_strategy="last_token", use_last_token_pooling=True
        )
    ...
    def test_qwen3_build_extra_body_disabled(self):
        ...
        provider = Qwen3EmbeddingProvider(model="qwen3", dimensions=2560, use_last_token_pooling=False)
    ...
    async def test_qwen3_get_embeddings_with_instructions(self, mock_ollama_response, sample_embeddings):
        ...
        with patch("embedding_providers.get_search_instruction", return_value="Search query:"):
            with patch("embedding_providers.format_query_for_qwen3", side_effect=lambda x: x):
                with patch(
                    "embedding_providers.get_detailed_instruct", side_effect=lambda task, text: f"{task} {text}"
                ):
                    provider = Qwen3EmbeddingProvider(model="qwen3", dimensions=4, use_instruction_format=True)
                    ...
                    result = await provider.get_embeddings_with_instructions(
                        ["text1", "text2"], task_description="Custom task"
                    )
...
class TestEmbeddingProviderFactory:
    ...
    def test_factory_create_with_config_override(self):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            provider = EmbeddingProviderFactory.create(
                "openai", api_key="test-key", model="custom-model", dimensions=512
            )
    ...
    def test_factory_create_from_env_openai(self):
        ...
        with patch.dict(
            os.environ,
            {"EMBEDDING_PROVIDER": "openai", "OPENAI_API_KEY": "test-key", "OPENAI_EMBEDDING_MODEL": "custom-model"},
        ):
    ...
    def test_factory_create_from_env_qwen3(self):
        ...
        with patch.dict(os.environ, {"EMBEDDING_PROVIDER": "ollama", "QWEN3_USE_INSTRUCTION_FORMAT": "true"}):
    ...
    async def test_get_embeddings_for_texts(self, sample_embeddings):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key", "EMBEDDING_PROVIDER": "openai"}):
            mock_result = EmbeddingResult(
                embeddings=sample_embeddings, model="test", dimensions=4, usage={}, provider="test"
            )
        ...
        with patch("embedding_providers.get_search_instruction", return_value="Search:"):
            with patch("embedding_providers.format_query_for_qwen3", side_effect=lambda x: x):
                with patch(
                    ...
                    provider = Qwen3EmbeddingProvider(model="qwen3", dimensions=100, use_instruction_format=True)
