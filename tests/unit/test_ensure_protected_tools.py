...
class TestEnsureProtectedTools:
    ...
    async def test_all_tools_already_attached(self):
        ...
        with patch("tool_manager.fetch_agent_tools", new_callable=AsyncMock) as mock_fetch:
            ...
            result = await ensure_protected_tools("agent-123", ["find_tools", "conversation_search"])
    ...
    async def test_attaches_missing_protected_tool(self):
        ...
        with (
            patch("tool_manager.fetch_agent_tools", new_callable=AsyncMock) as mock_fetch,
            ...
            result = await ensure_protected_tools("agent-123", ["find_tools"], available_tools=available_tools)
    ...
    async def test_partial_success_some_attached_some_failed(self):
        ...
        with (
            ...
            result = await ensure_protected_tools(
                "agent-123", ["existing_tool", "find_tools", "missing_tool"], available_tools=available_tools
            )
    ...
    async def test_fetch_agent_tools_failure(self):
        ...
        with patch("tool_manager.fetch_agent_tools", new_callable=AsyncMock) as mock_fetch:
            ...
            result = await ensure_protected_tools("agent-123", ["find_tools"])
