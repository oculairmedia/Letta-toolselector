...
def mock_letta_sdk_client_func():
    ...
    mock_client = Mock()
    mock_client.search_tools_with_scores = AsyncMock(
        return_value=[{"name": "tool1", "description": "A tool", "score": 0.9}]
    )
...
class TestRerankerConfig:
    ...
    def test_ollama_config(self):
        ...
        config = RerankerConfig(provider="ollama", url="http://ollama-adapter:8080/rerank")
...
class TestServiceConfiguration:
    ...
    def test_configure_basic(self, mock_weaviate_client):
        ...
        search_service.configure(weaviate_client=mock_weaviate_client)
    ...
    def test_configure_with_letta_sdk(self, mock_letta_sdk_client_func):
        ...
        search_service.configure(letta_sdk_client_func=mock_letta_sdk_client_func)
    ...
    def test_configure_with_custom_config(self, mock_weaviate_client):
        ...
        config = SearchConfig(provider="hybrid", reranker=RerankerConfig(enabled=False))

        search_service.configure(weaviate_client=mock_weaviate_client, config=config)
    ...
    def test_configure_creates_reranker(self, mock_weaviate_client):
        ...
        config = SearchConfig(reranker=RerankerConfig(enabled=True, provider="vllm"))
...
class TestVLLMReranker:
    ...
    async def test_rerank_success(self):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            results = await reranker.rerank(query="test query", documents=["doc1", "doc2"], top_k=2)
...
class TestOllamaReranker:
    ...
    async def test_rerank_success(self):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            results = await reranker.rerank(query="test", documents=["single doc"], top_k=1)
...
class TestSearch:
    ...
    async def test_search_letta_provider(self, mock_letta_sdk_client_func):
        ...
        search_service.configure(letta_sdk_client_func=mock_letta_sdk_client_func, config=config)
    ...
    async def test_search_hybrid_fallback(self, mock_weaviate_client):
        ...
        failing_sdk_client.search_tools_with_scores = AsyncMock(side_effect=RuntimeError("Letta failed"))
        failing_sdk_func = Mock(return_value=failing_sdk_client)

        config = SearchConfig(provider="hybrid")
        search_service.configure(
            weaviate_client=mock_weaviate_client, letta_sdk_client_func=failing_sdk_func, config=config
        )
...
class TestQueryExpansion:
    ...
    def test_expand_query_disabled(self, mock_weaviate_client):
        ...
        config = SearchConfig(expansion=QueryExpansionConfig(enabled=False))
        search_service.configure(weaviate_client=mock_weaviate_client, config=config)
    ...
    def test_expand_query_no_function(self, mock_weaviate_client):
        ...
        config = SearchConfig(expansion=QueryExpansionConfig(enabled=True))
        search_service.configure(weaviate_client=mock_weaviate_client, config=config)
...
class TestFormatToolForReranking:
    ...
    def test_basic_format(self):
        ...
        tool = {"name": "create_document", "description": "Creates a new document in the system"}
    ...
    def test_format_with_mcp_server(self):
        ...
        tool = {"name": "huly_create_issue", "description": "Create Huly issue", "mcp_server_name": "huly"}
    ...
    def test_format_with_mcp_tag(self):
        ...
        tool = {"name": "ghost_publish", "description": "Publish to Ghost", "tags": ["mcp:ghost", "blogging"]}
...
class TestRerankerTest:
    ...
    async def test_reranker_disabled(self, mock_weaviate_client):
        ...
        config = SearchConfig(reranker=RerankerConfig(enabled=False))
        search_service.configure(weaviate_client=mock_weaviate_client, config=config)
    ...
    async def test_reranker_not_initialized(self, mock_weaviate_client):
        ...
        config = SearchConfig(reranker=RerankerConfig(enabled=True))
        search_service.configure(weaviate_client=mock_weaviate_client, config=config)
...
class TestRerankResult:
    ...
    def test_create_rerank_result(self):
        ...
        result = RerankResult(index=0, relevance_score=0.95, document="Test document")
