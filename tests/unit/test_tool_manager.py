...
class TestAttachToolDirect:
    ...
    async def test_attach_tool_missing_id(self, test_agent_id):
        ...
        tool_manager.configure(http_session=MagicMock(), letta_url="http://test:8283", headers={}, use_letta_sdk=False)
    ...
    async def test_attach_tool_no_session(self, test_agent_id, sample_tool):
        ...
        tool_manager.configure(http_session=None, letta_url="http://test:8283", headers={}, use_letta_sdk=False)
...
class TestProcessToolsDirect:
    ...
    async def test_process_tools_respects_keep_list(self, test_agent_id):
        ...
        result = await tool_manager.process_tools(test_agent_id, current_tools, new_tools, keep_tools=["tool-keep"])
    ...
    async def test_process_tools_no_session_returns_error(self, test_agent_id):
        ...
        result = await tool_manager.process_tools(test_agent_id, [{"id": "tool-1"}], [{"id": "tool-2"}], keep_tools=[])
...
class TestFetchAgentToolsDirect:
    ...
    async def test_fetch_agent_tools_correct_url(self, test_agent_id):
        ...
        tool_manager.configure(
            http_session=mock_session, letta_url="http://test:8283", headers={"X-Custom": "header"}, use_letta_sdk=False
        )
