...
def main():
    ...
    attach_parser.add_argument(
        "--agent-id", "-a", default="agent-d5d91a6a-cc16-47dd-97be-07101cdbd49d", help="Agent ID to attach tools to"
    )
    attach_parser.add_argument("--limit", "-l", type=int, default=5, help="Maximum number of tools to return")
    attach_parser.add_argument(
        "--min-score", "-s", type=float, default=75.0, help="Minimum similarity score (0-100) for tools to be included"
    )
    ...
    if args.command == "attach-tools":
        ...
        payload = {"query": args.query, "agent_id": args.agent_id, "limit": args.limit, "min_score": args.min_score}
