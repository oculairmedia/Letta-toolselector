...
def load_config_from_env() -> AppConfig:
    ...
    # Build cache config
    cache_config = CacheConfig(cache_dir=os.getenv("CACHE_DIR", "/app/runtime_cache"))

    # Build webhook config
    webhook_config = WebhookConfig(matrix_bridge_url=os.getenv("MATRIX_BRIDGE_WEBHOOK_URL"))
...
def create_test_config(**overrides) -> AppConfig:
    ...
    """
    config = AppConfig(
        letta=LettaConfig(url="http://localhost:8283/v1", use_sdk=False),
        weaviate=WeaviateConfig(http_host="localhost", http_port=8080),
        search=SearchConfig(provider="weaviate", reranker=RerankerConfig(enabled=False)),
        tool_limits=ToolLimitsConfig(max_total_tools=30, max_mcp_tools=20),
        cache=CacheConfig(cache_dir="/tmp/test_cache"),
        testing=True,
    )
...
class ServiceContainer:
    ...
    def is_configured(self) -> bool:
        ...
        return self.search_service is not None or self.weaviate_client is not None
