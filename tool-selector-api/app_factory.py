...
from app_config import AppConfig, ServiceContainer, load_config_from_env, set_services, get_services
...
def create_app(config: Optional[AppConfig] = None) -> Quart:
    ...
    logger.info(
        f"Application created with config: "
        ...
        if config.letta.use_sdk:
            try:
                ...
                def get_letta_sdk_client():
                    return LettaSDKClient(base_url=config.letta.url, api_key=config.letta.api_key)
    ...
    try:
        ...
        search_config = SearchServiceConfig(
            ...
            expansion=SearchExpansionConfig(
                enabled=config.search.expansion.enabled, use_universal=config.search.expansion.use_universal
            ),
        )
...
async def _register_blueprints(app: Quart, config: AppConfig, services: ServiceContainer):
    ...
    from api_server import (
        read_tool_cache,
        read_mcp_servers_cache,
        process_matching_tool,
        log_config_change,
        perform_configuration_validation,
        test_service_connection,
        test_letta_connection,
        test_weaviate_connection as api_test_weaviate,
        get_tool_count_from_cache,
        get_cache_size,
        get_last_sync_time,
        get_weaviate_index_status,
        get_memory_usage,
        get_disk_usage,
        get_cpu_info,
        get_log_file_size,
        get_recent_error_count,
        get_recent_warning_count,
    ...
    try:
        ...
        async def get_health_status():
            ...
            return jsonify(
                {"status": "healthy", "weaviate": services.weaviate_client is not None, "cache_dir": cache_dir}
            )
    ...
    try:
        ...
        models_routes.configure(
            search_tools_func=search_tools, bm25_vector_override_service=bm25_vector_override_service
        )
