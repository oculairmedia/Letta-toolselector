...
async def search_and_attach_tools(
    tools: List[Dict[str, Any]], target_agent_id: str, session: aiohttp.ClientSession, min_score: float = 75.0
) -> Dict[str, Any]:
    ...
    try:
        ...
        for tool in tools:
            ...
            logger.debug(
                f"Tool: {tool_name} ({tool_id}), Distance: {distance:.4f}, Calculated Score: {match_score:.2f}%"
            )

            if match_score >= min_score:
                logger.info(
                    f"Tool {tool_name} ({tool_id}) passed threshold ({match_score:.2f}% >= {min_score:.2f}%). Adding for attachment."
                )
                ...
                logger.debug(
                    f"Tool {tool_name} ({tool_id}) failed threshold ({match_score:.2f}% < {min_score:.2f}%). Skipping."
                )

        if not tool_ids_to_attach:
            logger.warning(
                f"No tools met the minimum score threshold of {min_score}%. No attachments will be attempted."
            )
            return {
        ...
        for i, (response, tool_info) in enumerate(zip(responses, tools_to_attach_info)):
            ...
            if isinstance(response, Exception):
                ...
                successful.append({"tool_id": tool_id, "name": tool_name, "match_score": tool_info["match_score"]})
        ...
        if result["success"]:
            result["message"] = (
                f"Successfully processed {len(tools)} candidates, attached {len(successful)} tool(s) to agent {target_agent_id}"
            )
        else:
            result["message"] = (
                f"Processed {len(tools)} candidates, attempted {len(tool_ids_to_attach)} attachments. {len(successful)} succeeded, {len(failed)} failed."
            )
...
async def _run_async(query: str, target_agent_id: str, limit: int, min_score: float) -> Dict[str, Any]:
    ...
    async with AsyncExitStack() as stack:
        ...
        return await search_and_attach_tools(
            tools=candidate_tools, target_agent_id=target_agent_id, session=session, min_score=min_score
        )
...
def attach_tools_from_query(
    ...
    """
    logger.info(
        f"Received request for attach_tools_from_query. Request ID: {request_id}, Agent: {target_agent_id}, Query: '{query}'"
    )
    ...
    try:
        ...
        try:
            ...
            result = asyncio.run(
                _run_async(query=query, target_agent_id=target_agent_id, limit=limit, min_score=min_score)
            )
        ...
        return {"success": False, "message": f"Server error during tool attachment: {str(e)}", "request_id": request_id}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target-agent",
        required=True,  # Make required for standalone test
    ...
    parser.add_argument(
        "--min-score", type=float, default=75.0, help="Minimum similarity score (0-100) for tools to be included"
    )
