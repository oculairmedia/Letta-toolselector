...
class CostControlConfig:
    ...
    def __init__(self):
        ...
        self.pricing = {
            "openai_embedding_per_1k": float(
                os.getenv("OPENAI_EMBEDDING_COST_PER_1K", "0.0001")
        ...
        self.default_budgets = [
            ...
            BudgetLimit(
                CostCategory.EMBEDDING_API, BudgetPeriod.DAILY, float(os.getenv("DAILY_EMBEDDING_LIMIT", "5.0"))
            ),
        ]
...
class CostControlManager:
    ...
    def _get_period_bounds(
        self, period: BudgetPeriod, reference_time: Optional[datetime] = None
        ...
        if period == BudgetPeriod.HOURLY:
            ...
            start = reference_time.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(
    ...
    def _calculate_costs_for_period(
        self, category: Optional[CostCategory], period_start: datetime, period_end: datetime
    ) -> float:
        ...
        for budget_key, budget in self.budget_limits.items():
            ...
            if current_cost > budget.limit:
                ...
                alert = CostAlert(
                    ...
                    metadata={"period": budget.period.value, "hard_limit": budget.hard_limit, "budget_key": budget_key},
                )
                alerts.append(alert)
                ...
                for threshold in sorted(budget.alert_thresholds, reverse=True):
                    ...
                    if percentage_used >= threshold_pct:
                        ...
                        recent_alerts = [
                            a
                            for a in self.alerts_cache
                            if a.category == budget.category
                            and a.timestamp > period_start
                            and abs(a.percentage_used - percentage_used) < 5
                        ]  # Within 5%
    ...
    async def record_cost(
        self, category: CostCategory, operation: str, cost: float, metadata: Optional[Dict[str, Any]] = None
    ) -> bool:
        """
        ...
        with self._lock:
            # Create cost entry
            entry = CostEntry(
                timestamp=datetime.now(), category=category, operation=operation, cost=cost, metadata=metadata or {}
            )
    ...
    async def set_budget_limit(
        self, category: Optional[CostCategory], period: BudgetPeriod, limit: float, **kwargs
    ) -> str:
        """Set a budget limit"""
        budget = BudgetLimit(category=category, period=period, limit=limit, **kwargs)
    ...
    async def get_cost_summary(self, period: BudgetPeriod, reference_time: Optional[datetime] = None) -> CostSummary:
        ...
        relevant_entries = [entry for entry in self.cost_cache if period_start <= entry.timestamp < period_end]

        total_cost = sum(entry.cost for entry in relevant_entries)

        cost_by_category = {}
        for category in CostCategory:
            category_cost = sum(entry.cost for entry in relevant_entries if entry.category == category)
            if category_cost > 0:
    ...
    async def reset_period_costs(self, category: Optional[CostCategory], period: BudgetPeriod):
        ...
        with self._lock:
            self.cost_cache = [
                ...
                for entry in self.cost_cache
                if not (
                    period_start <= entry.timestamp < period_end and (category is None or entry.category == category)
                )
            ]
...
async def record_embedding_cost(operation: str, token_count: int, provider: str = "openai"):
    ...
    return await manager.record_cost(CostCategory.EMBEDDING_API, operation, cost, metadata)
...
async def record_weaviate_cost(operation: str, operation_type: str, count: int = 1):
    ...
    metadata = {"operation_type": operation_type, "count": count}

    return await manager.record_cost(CostCategory.VECTOR_DATABASE, operation, cost, metadata)
...
async def record_letta_api_cost(operation: str, call_count: int = 1):
    ...
    metadata = {"call_count": call_count, "cost_per_call": manager.config.pricing["letta_api_call_cost"]}

    return await manager.record_cost(CostCategory.LETTA_API, operation, cost, metadata)
