...
class OllamaEmbeddingProvider(EmbeddingProvider):
    ...
    async def get_embeddings(self, texts: List[str]) -> EmbeddingResult:
        ...
        try:
            ...
            response = await self.client.embeddings.create(model=self.model, input=texts, **request_kwargs)
...
class Qwen3EmbeddingProvider(OllamaEmbeddingProvider):
    ...
    def _build_extra_body(self) -> Optional[Dict[str, Any]]:
        if not self.use_last_token_pooling or not self.pooling_strategy:
            return None
        return {"options": {"pooling": self.pooling_strategy}, "pooling": self.pooling_strategy}
    ...
    async def get_embeddings_with_instructions(
        self, texts: List[str], task_description: Optional[str] = None
    ) -> EmbeddingResult:
    ...
    async def get_single_embedding(self, text: str, task_description: Optional[str] = None) -> List[float]:
...
class EmbeddingProviderFactory:
    ...
    _default_configs = {
        "openai": {"model": OPENAI_EMBEDDING_MODEL, "dimensions": OPENAI_EMBEDDING_DIMENSION},
        "ollama": {
    ...
    def create(cls, provider: str, **kwargs) -> EmbeddingProvider:
        ...
        if provider not in cls._providers:
            ...
            raise EmbeddingProviderError(f"Unknown provider '{provider}'. Available: {available}")
...
async def get_embeddings_for_texts(texts: List[str], provider: Optional[str] = None) -> EmbeddingResult:
...
async def test_providers():
    ...
    test_texts = ["This is a test sentence for embeddings.", "Another test sentence with different content."]
