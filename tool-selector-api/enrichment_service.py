...
class EnrichmentService:
    ...
    def __init__(
        self, anthropic_url: Optional[str] = None, anthropic_key: Optional[str] = None, cache_dir: Optional[str] = None
    ):
        """
        ...
        """
        self.anthropic_url = anthropic_url or os.getenv("ANTHROPIC_BASE_URL", "http://192.168.50.90:8082/v1")
        self.anthropic_key = anthropic_key or os.getenv("ANTHROPIC_API_KEY", "dummy")
        ...
        logger.info(
            f"EnrichmentService initialized: anthropic_url={self.anthropic_url}, cache_dir={self.cache.cache_dir}"
        )
    ...
    def anthropic_client(self) -> AnthropicClient:
        ...
        if self._anthropic_client is None:
            self._anthropic_client = AnthropicClient(base_url=self.anthropic_url, api_key=self.anthropic_key)
    ...
    def server_profiler(self) -> ServerProfiler:
        ...
        if self._server_profiler is None:
            self._server_profiler = ServerProfiler(client=self.anthropic_client, cache=self.cache)
    ...
    def tool_enricher(self) -> ToolEnricher:
        ...
        if self._tool_enricher is None:
            self._tool_enricher = ToolEnricher(client=self.anthropic_client, cache=self.cache)
    ...
    def group_tools_by_server(self, tools: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    async def run_enrichment(
        self, force_all: bool = False, max_tools: int = 50, max_servers: int = 10
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for server_name, server_tools in servers_to_profile:
                try:
                    ...
                    profile = await self.server_profiler.profile_server(server_name=server_name, tools=server_tools)
            ...
            for tool in tools_to_enrich:
                try:
                    ...
                    enriched = await self.tool_enricher.enrich_tool(tool=tool, server_profile=profile)
            ...
            for enriched in enriched_tools:
                try:
                    ...
                    result = collection.query.fetch_objects(
                        filters=wq.Filter.by_property("tool_id").equal(enriched.tool_id), limit=1
                    )
                    ...
                    if result.objects:
                        ...
                        collection.data.replace(uuid=obj.uuid, properties=merged_props)
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache": cache_stats,
            "config": {"anthropic_url": self.anthropic_url, "cache_dir": str(self.cache.cache_dir)},
        }
