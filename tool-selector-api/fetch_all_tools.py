            ...
            try:
                ...
                async with session.get(url, headers=headers, params=params, timeout=60) as response:
                    ...
                    if "application/json" in content_type:
                        ...
                        print(
                            f"Warning: Received non-JSON response ({content_type}) from {url}. Content: {text_response[:200]}..."
                        )
...
async def fetch_all_tools_async():
        ...
        if mcp_servers:
                ...
                for tool_data in mcp_tools_list:  # Rename loop variable
                    ...
                    if name not in tools_by_name:
                        ...
                        if "mcp_server_name" not in tools_by_name[name]:
                            ...
                            logger.warning(
                                f"Tool '{name}' found via multiple MCP servers: '{tools_by_name[name]['mcp_server_name']}' and '{server_name}'. Using first found."
                            )
                    ...
                    async with session.post(register_url, headers=headers, timeout=60) as register_response:
                        if register_response.status == 200:
                            ...
                            if registered_tool and "id" in registered_tool and "name" in registered_tool:
                                print(
                                    f"Successfully registered MCP tool '{registered_tool['name']}' with ID {registered_tool['id']}"
                                )
                                return registered_tool  # Return the registered tool data
                            else:
                                print(
                                    f"Warning: Registration response for '{tool_name}' missing tool ID or name. Response: {await register_response.text()}"
                                )
                            ...
                            print(
                                f"Warning: Failed to register '{tool_name}' from '{server_name}'. Status: {register_response.status}. Response: {error_text[:500]}"
                            )
            ...
            for result in registration_results:
                if isinstance(result, Exception):
                    ...
                    original_tool_data = next(
                        (t_data for srv, t_name, t_data in tools_to_register if t_name == registered_tool_name), None
                    )
                    ...
                    if original_tool_data:
                        ...
                        logger.warning(
                            f"Could not find original data for successfully registered tool: {registered_tool_name}. Storing raw API result."
                        )
    ...
    for name, tool_data in tools_by_name.items():
        ...
        logger.debug(
            f"Filtering check for tool '{name}': Type='{tool_data.get('tool_type')}', Server='{tool_data.get('mcp_server_name')}'"
        )
        if tool_data.get("tool_type") == "external_mcp":
            ...
            if not server_origin:
                logger.warning(
                    f"Excluding tool '{name}' because it is external_mcp but has no mcp_server_name recorded (likely obsolete)."
                )
                removed_count += 1
                continue  # Skip adding this tool
            elif server_origin not in active_server_names:
                logger.warning(
                    f"Excluding tool '{name}' because its MCP server '{server_origin}' is no longer listed as active."
                )
                removed_count += 1
                continue  # Skip adding this tool
            # --- End Stricter Filter ---
...
def categorize_tools(tools):
    ...
    for tool in tools:
        ...
        if category not in categories:
            categories[category] = []
        categories[category].append({"name": name, "source_type": source_type, "tool_type": tool_type})
