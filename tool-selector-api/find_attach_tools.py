...
def find_attach_tools(
    ...
    logger.debug(
        f"Parameters - limit: {limit}, min_score: {min_score}, heartbeat: {request_heartbeat}, url: {weaviate_base}"
    )
    ...
    try:
        ...
        detach_results = {"detached_tools": [], "failed_tools": []}
        ...
        for tool in mcp_tools:
            ...
            try:
                ...
                detach_results["detached_tools"].append(
                    {"tool_id": tool_id, "name": tool_name, "type": tool.get("tool_type")}
                )
                ...
                detach_results["failed_tools"].append({"tool_id": tool_id, "name": tool_name, "error": error_msg})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--min_score", type=float, default=75.0, help="Minimum similarity score (0-100) - Used server-side"
    )
    parser.add_argument(
        "--request_heartbeat", action="store_true", help="(Currently unused in this script) Request immediate heartbeat"
    )
    parser.add_argument(
        "--debug_level", default="INFO", choices=["DEBUG", "INFO", "WARNING", "ERROR"], help="Set logging verbosity"
    )
