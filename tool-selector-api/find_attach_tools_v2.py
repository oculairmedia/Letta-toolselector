                ...
                for tool in mcp_tools:
                    try:
                        detach_response = session.patch(
                            f"{letta_base}/agents/{target_agent_id}/tools/detach/{tool['id']}", headers=headers
                        )
                        ...
                        detach_results["failed_tools"].append({"tool_id": tool["id"], "name": tool["name"]})

        # 2. Search and attach new tools
        search_payload = {"q": query, "size": limit, "threshold": min_score, "agent": target_agent_id}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target_agent_id", dest="target_agent_id", default=None, help="ID of the target agent (optional)."
    )
    parser.add_argument("--limit", type=int, default=5, help="Maximum number of tools to find (default: 5).")
    parser.add_argument(
        "--min_score", type=float, default=75.0, help="Minimum similarity score (0-100) (default: 75.0)."
    )
