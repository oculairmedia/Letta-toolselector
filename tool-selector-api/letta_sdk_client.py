...
class LettaSDKClient:
    ...
    def __init__(
        self, base_url: Optional[str] = None, api_key: Optional[str] = None, max_retries: int = 3, timeout: float = 30.0
    ):
        """
        ...
        custom_headers = {"X-BARE-PASSWORD": f"password {self._api_key}"}

        self._client: Letta = Letta(
            base_url=self._base_url, default_headers=custom_headers, max_retries=max_retries, timeout=timeout
        )
    ...
    async def get_agent(self, agent_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            agent = await self._run_sync(self._client.agents.retrieve, agent_id=agent_id)
    ...
    async def list_agent_tools(self, agent_id: str) -> List[Dict[str, Any]]:
        ...
        """
        try:
            tools = await self._run_sync(self._client.agents.tools.list, agent_id=agent_id)
    ...
    async def attach_tool(self, agent_id: str, tool_id: str, tool_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            result = await self._run_sync(self._client.agents.tools.attach, agent_id=agent_id, tool_id=tool_id)
            ...
            return {"success": False, "tool_id": tool_id, "name": tool_name, "error": error_msg}

    async def detach_tool(self, agent_id: str, tool_id: str, tool_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        tool_name = tool_name or tool_id
        try:
            await self._run_sync(self._client.agents.tools.detach, agent_id=agent_id, tool_id=tool_id)
            logger.debug(f"Successfully detached tool {tool_name} ({tool_id}) from agent {agent_id}")
            return {"success": True, "tool_id": tool_id, "name": tool_name}
        except Exception as e:
            error_msg = str(e)
    ...
    async def refresh_agent_tools(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._run_sync(self._client.agents.update, agent_id=agent_id, tool_ids=tool_ids)

            logger.info(f"Refreshed agent {agent_id} tool list with {len(tool_ids)} tools")
            return {"success": True, "agent_id": agent_id, "tool_count": len(tool_ids), "tool_names": tool_names}
        except Exception as e:
            error_msg = str(e)
            logger.error(f"Failed to refresh agent {agent_id} tools: {error_msg}")
            return {"success": False, "agent_id": agent_id, "error": error_msg}
    ...
    async def search_tools(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            tools = await self._run_sync(self._client.tools.search, query=query)
    ...
    async def register_mcp_tool(self, tool_name: str, server_name: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            result = await self._run_sync(
                self._client.tools._post, f"/tools/mcp/servers/{server_name}/{tool_name}", cast_to=dict
            )
    ...
    async def batch_attach_tools(self, agent_id: str, tools: List[Dict[str, Any]]) -> Dict[str, List]:
        """
    ...
    async def batch_detach_tools(
        self, agent_id: str, tool_ids: List[str], tool_names: Optional[Dict[str, str]] = None
    ) -> Dict[str, List]:
        """
        ...
        tasks = [self.detach_tool(agent_id, tid, tool_names.get(tid)) for tid in tool_ids]
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": result.status, "version": result.version, "healthy": result.status == "ok"}
        except Exception as e:
