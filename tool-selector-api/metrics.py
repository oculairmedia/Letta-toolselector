...
if PROMETHEUS_AVAILABLE:
    # Request metrics
    REQUEST_COUNT = Counter("tool_selector_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
    ...
    ATTACH_TOOLS = Histogram(
        "tool_selector_attach_tools_count", "Number of tools attached per operation", buckets=[0, 1, 2, 5, 10, 20, 50]
    )
    ...
    PRUNE_TOOLS = Histogram(
        "tool_selector_prune_tools_count", "Number of tools pruned per operation", buckets=[0, 1, 2, 5, 10, 20, 50]
    )

    PRUNE_AGENTS = Histogram(
        "tool_selector_prune_agents_count", "Number of agents processed per prune run", buckets=[1, 5, 10, 25, 50, 100]
    )

    # Weaviate connection pool metrics
    WEAVIATE_POOL_SIZE = Gauge("tool_selector_weaviate_pool_size", "Current Weaviate connection pool size")

    WEAVIATE_POOL_ACTIVE = Gauge("tool_selector_weaviate_pool_active", "Active Weaviate connections")

    WEAVIATE_POOL_WAITS = Counter(
        "tool_selector_weaviate_pool_waits_total", "Total times requests waited for a connection"
    )

    WEAVIATE_POOL_TIMEOUTS = Counter(
        "tool_selector_weaviate_pool_timeouts_total", "Total connection pool timeout errors"
    )
    ...
    # API info
    API_INFO = Info("tool_selector", "Tool Selector API information")
...
def set_api_info(version: str = "1.0.0", environment: str = "production"):
    ...
    if PROMETHEUS_AVAILABLE:
        API_INFO.info({"version": version, "environment": environment})
