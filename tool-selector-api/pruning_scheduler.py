...
class PruningScheduler:
    ...
    async def run_now(self, dry_run: Optional[bool] = None) -> SchedulerRunResult:
        ...
        if not self.is_configured():
            return SchedulerRunResult(
                started_at=datetime.now(), completed_at=datetime.now(), error="Scheduler not configured"
            )

        use_dry_run = dry_run if dry_run is not None else self.config.dry_run

        result = SchedulerRunResult(started_at=datetime.now(), dry_run=use_dry_run)
        ...
        try:
            ...
            if self._list_agents is None:
                return SchedulerRunResult(
                    started_at=datetime.now(), completed_at=datetime.now(), error="list_agents function not configured"
                )
            ...
            for i in range(0, len(agents), self.config.batch_size):
                ...
                for agent in batch:
                    ...
                    agent_result = await self._prune_single_agent(
                        agent_id=agent_id, agent_name=agent_name, dry_run=use_dry_run
                    )
    ...
    async def _prune_single_agent(self, agent_id: str, agent_name: str, dry_run: bool) -> PruningResult:
        """Prune MCP tools from a single agent."""
        result = PruningResult(agent_id=agent_id, success=False, dry_run=dry_run)
        ...
        try:
            ...
            prune_result = self._prune_agent(
                agent_id=agent_id, user_prompt=context, drop_rate=self.config.drop_rate, dry_run=dry_run
            )
            ...
            if prune_result.get("success"):
                ...
                result.mcp_tools_after = details.get("mcp_tools_on_agent_before", 0) - details.get(
