...
class QueryExpander:
    ...
    def get_multifunctional_tool_keywords(self, operations: List[OperationType], domains: List[str]) -> Set[str]:
        """
        ...
        for tool_name, tool_info in KNOWN_MULTIFUNCTIONAL_TOOLS.items():
            ...
            ops_match = any(
                op.value in tool_ops or op.value.lower() in tool_ops for op in operations if op != OperationType.UNKNOWN
            )
