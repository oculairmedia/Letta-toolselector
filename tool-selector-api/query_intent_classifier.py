...
ACTION_PATTERNS: Dict[ActionType, Dict[str, Any]] = {
    ...
    ActionType.CONNECT: {
        "verbs": {"connect", "link", "attach", "bind", "associate", "join", "integrate", "sync", "synchronize"},
        "patterns": [
...
DOMAIN_KEYWORDS: Dict[str, Set[str]] = {
    ...
    "ai_agents": {"agent", "letta", "memory", "tool", "assistant", "bot", "archival", "conversation", "context"},
    "data_management": {"database", "record", "table", "query", "graphiti", "weaviate", "neo4j", "postgres", "schema"},
}
...
class QueryIntentClassifier:
        ...
        for action_type, config in ACTION_PATTERNS.items():
            for pattern in config.get("patterns", []):
                for match in re.finditer(pattern, query):
                    # Avoid duplicates
                    already_found = any(a[1] in match.group(0) for a in actions)
    ...
    def _extract_entities(self, query: str) -> List[EntityReference]:
        ...
        for word in words:
            ...
            if entity_type:
                ...
                entities.append(EntityReference(entity_type=entity_type, qualifiers=qualifiers))
    ...
    def _determine_complexity(self, query: str, actions: List[Tuple[ActionType, str, int]]) -> QueryComplexity:
        """Determine query complexity"""

        # Check for compound indicators
        has_compound = any(re.search(pattern, query) for pattern in COMPOUND_INDICATORS)
    ...
    def _match_actions_to_entities(
        self, actions: List[Tuple[ActionType, str, int]], entities: List[EntityReference], query: str
    ) -> List[ActionIntent]:
        ...
        for action_type, verb, pos in actions:
            ...
            matched.append(
                ActionIntent(action=action_type, target_entity=target_entity, confidence=confidence, source_phrase=verb)
            )
        ...
        if not matched and entities:
            ...
            matched.append(
                ActionIntent(
                    action=ActionType.LIST, target_entity=entities[0], confidence=0.5, source_phrase="(inferred)"
                )
            )
    ...
    def _generate_boost_keywords(
        self, actions: List[ActionIntent], entities: List[EntityReference], domains: List[str]
    ) -> List[str]:
