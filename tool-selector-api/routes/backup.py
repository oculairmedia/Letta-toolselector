...
def configure(
    cache_dir=None, log_config_change=None, perform_configuration_validation=None, test_service_connection=None
):
    """
...
async def list_config_backups():
    ...
    try:
        ...
        if not os.path.exists(backup_dir):
            return jsonify({"success": True, "data": {"backups": [], "total": 0}})
        ...
        return jsonify({"success": True, "data": {"backups": backups, "total": len(backups)}})
                    ...
                    for key, value in config_data.items():
                        if isinstance(value, dict):
                            for sub_key in list(value.keys()):
                                if (
                                    "key" in sub_key.lower()
                                    or "password" in sub_key.lower()
                                    or "secret" in sub_key.lower()
                                ):
                                    value[sub_key] = "***"
                        elif "key" in key.lower() or "password" in key.lower() or "secret" in key.lower():
                            config_data[key] = "***"

        return jsonify({"success": True, "data": masked_data})
            ...
            if config_name == "tool_selector":
                restoration_plan[config_name] = {
                    ...
                    "changes": [
                        {
                            "var": "MAX_TOTAL_TOOLS",
                            "current": os.getenv("MAX_TOTAL_TOOLS", "30"),
                            "new": str(config_data["tool_limits"]["max_total_tools"]),
                        },
                        {
                            "var": "MAX_MCP_TOOLS",
                            "current": os.getenv("MAX_MCP_TOOLS", "20"),
                            "new": str(config_data["tool_limits"]["max_mcp_tools"]),
                        },
                        {
                            "var": "MIN_MCP_TOOLS",
                            "current": os.getenv("MIN_MCP_TOOLS", "7"),
                            "new": str(config_data["tool_limits"]["min_mcp_tools"]),
                        },
                        {
                            "var": "DEFAULT_DROP_RATE",
                            "current": os.getenv("DEFAULT_DROP_RATE", "0.6"),
                            "new": str(config_data["behavior"]["default_drop_rate"]),
                        },
                        {
                            "var": "EXCLUDE_LETTA_CORE_TOOLS",
                            "current": os.getenv("EXCLUDE_LETTA_CORE_TOOLS", "true"),
                            "new": str(config_data["behavior"]["exclude_letta_core_tools"]).lower(),
                        },
                    ],
                }
                ...
                restoration_plan[config_name] = {
                    ...
                    "changes": [
                        {
                            "var": "OLLAMA_EMBEDDING_HOST",
                            "current": os.getenv("OLLAMA_EMBEDDING_HOST", "192.168.50.80"),
                            "new": config_data["connection"]["host"],
                        },
                        {
                            "var": "OLLAMA_PORT",
                            "current": os.getenv("OLLAMA_PORT", "11434"),
                            "new": str(config_data["connection"]["port"]),
                        },
                        {
                            "var": "OLLAMA_EMBEDDING_MODEL",
                            "current": os.getenv("OLLAMA_EMBEDDING_MODEL", ""),
                            "new": config_data["embedding"]["model"],
                        },
                        {
                            "var": "USE_OLLAMA_EMBEDDINGS",
                            "current": os.getenv("USE_OLLAMA_EMBEDDINGS", "false"),
                            "new": str(config_data["embedding"]["enabled"]).lower(),
                        },
                    ],
                }
                ...
                restoration_plan[config_name] = {
                    ...
                    "changes": [
                        {
                            "var": "WEAVIATE_URL",
                            "current": os.getenv("WEAVIATE_URL", ""),
                            "new": config_data["connection"]["url"],
                        },
                        {
                            "var": "WEAVIATE_TIMEOUT",
                            "current": os.getenv("WEAVIATE_TIMEOUT", "30"),
                            "new": str(config_data["connection"]["timeout"]),
                        },
                        {
                            "var": "WEAVIATE_ALPHA",
                            "current": os.getenv("WEAVIATE_ALPHA", "0.75"),
                            "new": str(config_data["search"]["alpha"]),
                        },
                    ],
                }
                ...
                if "api_key" in config_data["connection"]:
                    restoration_plan[config_name]["changes"].append(
                        {
                            "var": "WEAVIATE_API_KEY",
                            "current": "***" if os.getenv("WEAVIATE_API_KEY") else None,
                            "new": "***",
                        }
                    )
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "warnings": warnings
                    + ["Restoration is currently simulated - actual environment variable updates not implemented"],
                },
            }
        )
        ...
        return jsonify(
            {
                ...
                "data": {"backup_name": backup_name, "deleted_at": datetime.now().isoformat()},
            }
        )
...
async def list_saved_configurations():
    ...
    try:
        ...
        if not os.path.exists(saves_dir):
            return jsonify({"success": True, "data": {"saves": [], "total": 0}})
        ...
        return jsonify({"success": True, "data": {"saves": saves, "total": len(saves)}})
    ...
    try:
        ...
        if reset_type == "defaults":
            ...
            default_configs = {
                ...
                "letta_api": {"LETTA_API_URL": "https://letta.example.com/v1", "LETTA_TIMEOUT": "30"},
            }
            ...
            for section, config in saved_configs.items():
                reset_actions.append(
                    {"action": "reset_to_saved", "section": section, "config_name": config_name, "config": config}
                )
        else:
        ...
        if not dry_run and _log_config_change:
            await _log_config_change(
                ...
                config_type="system_wide",
                changes={"reset_type": reset_type, "config_name": config_name, "sections": sections},
                metadata={"actions_count": len(reset_actions), "warnings": warnings},
            )
...
async def validate_connection():
    ...
    try:
        ...
        return jsonify({"success": True, "data": connection_result})
    ...
    try:
        ...
        for validation in validations:
            try:
                ...
                results.append(
                    {"config_type": validation.get("config_type"), "field": validation.get("field"), "result": result}
                )
            except Exception as e:
                results.append(
                    {
...
async def get_config_audit_logs():
    ...
    try:
        ...
        if not os.path.exists(audit_file):
            return jsonify(
                {"success": True, "data": {"logs": [], "total": 0, "filtered": 0, "offset": offset, "limit": limit}}
            )
...
async def clear_config_audit_logs():
    ...
    try:
        ...
        if not confirm:
            return jsonify(
                {"success": False, "error": "Confirmation required. Set 'confirm': true in request body."}
            ), 400
        ...
        return jsonify(
            {
                ...
                "data": {"cleared_at": datetime.now().isoformat(), "backup_created": backup_file},
            }
        )
