...
async def create_benchmark_query_set():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "data": new_query_set, "message": f"Query set '{data['name']}' created successfully"}
        )
...
async def get_benchmark_query_set(query_set_id):
    ...
    try:
        ...
        return jsonify({"success": True, "data": query_set})
...
async def update_benchmark_query_set(query_set_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "data": query_set, "message": f"Query set {query_set_id} updated successfully"}
        )
...
async def delete_benchmark_query_set(query_set_id):
    ...
    try:
        ...
        benchmark_data["query_sets"] = [qs for qs in benchmark_data.get("query_sets", []) if qs["id"] != query_set_id]
        ...
        return jsonify({"success": True, "message": f"Query set {query_set_id} deleted successfully"})
...
async def run_benchmark_query_set(query_set_id):
    ...
    try:
                ...
                if _search_tools:
                    if config:
                        search_results = await asyncio.to_thread(
                            _search_tools, query=query_text, limit=limit, reranker_config=config
                        )
                    else:
                        search_results = await asyncio.to_thread(_search_tools, query=query_text, limit=limit)
                else:
                    search_results = []
        ...
        avg_precision = sum(r["metrics"].get("precision_at_k", 0) for r in successful_queries) / max(
            len(successful_queries), 1
        )
        avg_recall = sum(r["metrics"].get("recall_at_k", 0) for r in successful_queries) / max(
            len(successful_queries), 1
        )
        ...
        logger.info(
            f"Completed benchmark run for {query_set_id}: {len(successful_queries)}/{len(results)} queries successful"
        )
...
async def get_benchmark_run_history():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "data": {"runs": runs, "total": len(runs), "filtered_by_query_set": query_set_id}}
        )
