...
def configure(
    get_tools_by_server_func: Optional[Callable] = None, get_all_tools_func: Optional[Callable] = None
) -> None:
...
async def enrichment_health():
    ...
    try:
        ...
        return jsonify({"success": True, "cache": {"healthy": True, **cache_stats}, "api": api_status})
    except Exception as e:
...
async def profile_server(server_name: str):
    ...
    try:
        ...
        if not _get_tools_by_server_func:
            return jsonify({"success": False, "error": "get_tools_by_server function not configured"}), 503

        servers_tools = _get_tools_by_server_func()
        tools = servers_tools.get(server_name, [])

        if not tools:
            return jsonify({"success": False, "error": f"No tools found for server: {server_name}"}), 404
...
async def get_server_profile_status(server_name: str):
    ...
    try:
        ...
        if profile:
            ...
            return jsonify({"success": True, "exists": False, "needs_update": True})
...
async def enrich_tool(tool_id: str):
    ...
    try:
        ...
        if not _get_all_tools_func:
            return jsonify({"success": False, "error": "get_all_tools function not configured"}), 503

        all_tools = _get_all_tools_func()
        tool = next((t for t in all_tools if t.get("id") == tool_id or t.get("tool_id") == tool_id), None)

        if not tool:
            return jsonify({"success": False, "error": f"Tool not found: {tool_id}"}), 404
...
async def run_enrichment_sync():
    ...
    try:
        ...
        stats = EnrichmentStats(total_servers=len(servers_tools), started_at=datetime.utcnow())
...
async def sync_enrichment_to_weaviate():
    ...
    try:
        ...
        return jsonify({"success": False, "error": "Sync timed out after 5 minutes"}), 504
    except Exception as e:
...
async def clear_enrichment_cache():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Enrichment cache cleared"})
