...
def calculate_search_metrics(
    search_results: List[Dict], expected_tools: List[str], k_values: List[int]
) -> Dict[str, Any]:
...
def simplified_significance_test(
    values_a: List[float], values_b: List[float], confidence_level: float
) -> Dict[str, Any]:
...
def perform_significance_test(
    values_a: List[float], values_b: List[float], confidence_level: float, min_effect_size: float
) -> Dict[str, Any]:
    ...
    pooled_std = math.sqrt(
        ((len(values_a) - 1) * calculate_std(values_a) ** 2 + (len(values_b) - 1) * calculate_std(values_b) ** 2)
        / (len(values_a) + len(values_b) - 2)
    )

    effect_size = (
        abs(sum(values_b) / len(values_b) - sum(values_a) / len(values_a)) / pooled_std if pooled_std > 0 else 0.0
    )
    ...
    power = 1 - stats.t.cdf(
        t_critical - effect_size * math.sqrt(len(values_a) * len(values_b) / (len(values_a) + len(values_b)) / 2),
        len(values_a) + len(values_b) - 2,
    )
...
def calculate_overall_comparison_stats(
    results_a: List[Dict], results_b: List[Dict], metrics: List[str], k_values: List[int], confidence_level: float
) -> Dict[str, Any]:
...
async def execute_ab_comparison(
    ...
    for metric in metrics:
        for k in k_values:
            ...
            stat_result = perform_significance_test(values_a, values_b, confidence_level, min_effect_size)
            statistical_results[metric_key] = stat_result

    # Calculate overall effect sizes and power analysis
    overall_stats = calculate_overall_comparison_stats(results_a, results_b, metrics, k_values, confidence_level)
...
async def get_evaluations():
    ...
    try:
        ...
        return jsonify({"success": True, "data": evaluations, "total": len(evaluations)})
    except Exception as e:
...
async def compare_rerank():
    ...
    try:
        ...
        comparison_metrics = {
            ...
            "top_5_overlap": calculate_overlap(
                [r["tool"]["id"] for r in results_a[:5]], [r["tool"]["id"] for r in results_b[:5]]
            ),
...
async def get_ab_results():
    ...
    try:
        ...
        for filename in os.listdir(ab_comparison_dir):
            if filename.endswith(".json"):
                try:
                    ...
                    if config_name:
                        if (
                            config_name.lower() not in result.get("config_a", {}).get("name", "").lower()
                            and config_name.lower() not in result.get("config_b", {}).get("name", "").lower()
                        ):
                            continue

                    if min_significance is not None:
                        max_p_value = max(
                            [stat.get("p_value", 1.0) for stat in result.get("statistical_results", {}).values()]
                        )
        ...
        return jsonify(
            {"success": True, "data": {"results": paginated_results, "total": total, "offset": offset, "limit": limit}}
        )
