...
def configure(search_tools_func: Optional[Callable] = None, bm25_vector_override_service: Optional[Any] = None):
    """
...
async def get_embedding_models():
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "data": {"models": models, "total": len(models), "providers": list(set(m["provider"] for m in models))},
            }
        )
                    ...
                    if response.status == 200:
                        ...
                        for model in ollama_data.get("models", []):
                            ...
                            if any(
                                ...
                                for keyword in ["rerank", "cross-encoder", "bge-rerank", "colbert"]
                            ):
                                models.append(
                                    {
                                        "id": model_name,
        ...
        try:
            if embedding_provider == "openai":
                ...
                response = await asyncio.to_thread(
                    openai.embeddings.create, input="test embedding health check", model=embedding_model
                )
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    payload = {"model": embedding_model, "prompt": "test embedding health check"}
...
async def search_test():
    ...
    try:
        ...
        reranker_overrides = {"enabled": reranker_enabled}
        if reranker_model:
        ...
        for i, result in enumerate(search_results):
            formatted_result = {
                ...
                if reranker_enabled and result.get("rerank_score") is not None
