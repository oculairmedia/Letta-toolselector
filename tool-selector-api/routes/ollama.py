...
async def get_ollama_models():
    ...
    try:
            ...
            async with session.get(ollama_url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status == 200:
                    ...
                    return jsonify(
                        {
                            "success": True,
                            "data": {"models": formatted_models, "base_url": base_url, "total": len(formatted_models)},
                        }
                    )
                else:
        ...
        return jsonify(
            {"success": False, "error": f"Failed to fetch models: {str(e)}", "fallback_models": _get_fallback_models()}
        ), 503
