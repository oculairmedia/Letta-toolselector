...
async def get_maintenance_status():
    ...
    try:
        maintenance_status = {
            ...
            "services": {
                "weaviate": await _test_weaviate_connection({"url": os.getenv("WEAVIATE_URL", "http://weaviate:8080/")})
                if _test_weaviate_connection
                else {"available": False},
                "ollama": await _test_ollama_connection({"host": os.getenv("OLLAMA_EMBEDDING_HOST", "192.168.50.80")})
...
async def perform_system_cleanup():
    ...
    try:
        ...
        cleanup_results = {"operations": [], "total_space_freed": 0, "errors": []}
...
async def restart_system_components():
    ...
    try:
        ...
        if not components:
            return jsonify({"success": False, "error": "No components specified for restart"}), 400

        restart_results = {"components": [], "warnings": []}
...
async def optimize_system():
    ...
    try:
        ...
        optimization_results = {"operations": [], "performance_impact": {}, "warnings": []}
        ...
        return jsonify(
            {
                "success": True,
                "data": {"optimization_results": optimization_results, "completed_at": datetime.now().isoformat()},
            }
        )
    ...
    try:
        ...
        if _get_log_entries:
            log_entries = await _get_log_entries(
                level=level, lines=lines, search=search, from_time=from_time, to_time=to_time
            )
        else:
            log_entries = []
    ...
    try:
        ...
        if _get_error_log_entries:
            error_logs = await _get_error_log_entries(
                hours=hours, include_stack_trace=include_stack_trace, group_by=group_by
            )
        else:
            error_logs = {"errors": [], "message": "Error log retrieval not configured"}

        return jsonify({"success": True, "data": error_logs})
    except Exception as e:
...
async def export_logs():
    ...
    try:
        ...
        return jsonify({"success": True, "data": export_result})
    except Exception as e:
...
SAFE_ENV_VARS = {
    ...
    "MIN_MCP_TOOLS",
    "DEFAULT_DROP_RATE",
    "EXCLUDE_LETTA_CORE_TOOLS",
    "EXCLUDE_OFFICIAL_TOOLS",
    "MANAGE_ONLY_MCP_TOOLS",
    "MIN_SCORE_DEFAULT",
    "SEMANTIC_WEIGHT",
    "KEYWORD_WEIGHT",
    ...
    "WEAVIATE_RETRIES",
    "WEAVIATE_CLASS_NAME",
    "WEAVIATE_VECTOR_INDEX",
    "WEAVIATE_DISTANCE_METRIC",
    "WEAVIATE_ALPHA",
    "WEAVIATE_LIMIT",
    "WEAVIATE_AUTOCUT",
    "WEAVIATE_EF_CONSTRUCTION",
    "WEAVIATE_EF",
    "WEAVIATE_MAX_CONNECTIONS",
    "WEAVIATE_VECTOR_CACHE_MAX_OBJECTS",
    "WEAVIATE_CLEANUP_INTERVAL",
    ...
    try:
        ...
        for var, value in env_vars.items():
            if (
                ...
                or var.startswith("DEFAULT_")
                or var.startswith("EXCLUDE_")
                or var.startswith("MANAGE_")
                or var in ["SEMANTIC_WEIGHT", "KEYWORD_WEIGHT"]
            ):
        ...
        return jsonify(
            {"success": True, "data": {"variables": env_vars, "grouped": grouped_vars, "total_count": len(env_vars)}}
        )
    except Exception as e:
...
async def validate_environment():
    ...
    try:
        ...
        return jsonify({"success": True, "data": {"results": validation_results, "all_valid": all_valid}})
    except Exception as e:
