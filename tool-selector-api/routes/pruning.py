...
async def start_scheduler():
    ...
    if scheduler._running:
        return jsonify({"success": True, "message": "Scheduler already running", "status": scheduler.get_status()})

    # Enable if not already
    scheduler.config.enabled = True
    await scheduler.start()

    return jsonify({"success": True, "message": "Scheduler started", "status": scheduler.get_status()})
...
async def stop_scheduler():
    ...
    if not scheduler._running:
        return jsonify({"success": True, "message": "Scheduler not running", "status": scheduler.get_status()})

    await scheduler.stop()

    return jsonify({"success": True, "message": "Scheduler stopped", "status": scheduler.get_status()})
