...
def run_enrichment(
    ...
    client = AnthropicClient(
        base_url=anthropic_base_url, api_key=anthropic_api_key, model="claude-sonnet-4-20250514", timeout=120.0
    )
    ...
    logger.info("=" * 60)
    logger.info(
        f"Servers: {stats.servers_profiled} profiled, {stats.servers_cached} cached, {stats.servers_failed} failed"
    )
...
def main():
    ...
    parser.add_argument(
        "--tool-cache", default="/app/runtime_cache/tool_cache.json", help="Path to tool cache JSON file"
    )
    parser.add_argument(
        "--anthropic-url",
    ...
    parser.add_argument("--anthropic-key", default=os.getenv("ANTHROPIC_API_KEY", ""), help="Anthropic API key")
    parser.add_argument("--force", action="store_true", help="Force re-enrichment of all content")
    parser.add_argument(
        "--servers-only", action="store_true", help="Only profile servers, don't enrich individual tools"
    )
    parser.add_argument("--server", help="Only process this specific server (for testing)")
