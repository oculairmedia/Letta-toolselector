...
class VLLMReranker:
    ...
    def __init__(
        self, url: str = "http://100.81.139.20:11435/rerank", model: str = "qwen3-reranker-4b", timeout: float = 30.0
    ):
        self.url = url
        self.model = model
    ...
    async def rerank(self, query: str, documents: List[str], top_k: int = 10) -> List[RerankResult]:
        ...
        payload = {"model": self.model, "query": query, "documents": documents, "top_k": min(top_k, len(documents))}
...
def create_reranker(config: RerankerConfig) -> Optional[RerankerClient]:
    ...
    if config.provider.lower() == "vllm":
        return VLLMReranker(url=config.url, model=config.model, timeout=config.timeout)
    elif config.provider.lower() == "ollama":
        return OllamaReranker(url=config.url, timeout=config.timeout, instruction=config.instruction)
...
def configure(
    ...
    logger.info(
        f"Search service configured: provider={_config.provider}, "
...
def _import_query_expansion():
    ...
    if _config.expansion.use_universal:
        try:
            from universal_query_expansion import expand_query_universally, expand_query_with_analysis
...
async def _search_via_letta(query: str, limit: int, min_score: float) -> List[Dict[str, Any]]:
    ...
    results = await sdk_client.search_tools_with_scores(query=query, limit=limit, min_score=min_score)
...
async def _search_via_weaviate(query: str, limit: int, use_reranking: bool) -> List[Dict[str, Any]]:
    ...
    results = await asyncio.to_thread(
        search_tools_with_reranking, query=query, limit=limit, use_reranking=use_reranking
    )
    logger.info(f"Weaviate search for '{query}' returned {len(results)} results")
    return results


async def rerank(query: str, documents: List[str], top_k: int = 10) -> List[RerankResult]:
    """
...
async def test_reranker() -> Dict[str, Any]:
    ...
    try:
        ...
        test_docs = ["Tool for creating new documents", "Tool for searching files", "Tool for deleting records"]
