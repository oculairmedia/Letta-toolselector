...
class AnthropicClient:
    ...
    def __init__(
        ...
        self.base_url = base_url or os.getenv(
            "ANTHROPIC_PROXY_URL", os.getenv("ANTHROPIC_BASE_URL", "http://192.168.50.90:8082/v1")
        )
        ...
        logger.info(f"AnthropicClient initialized: base_url={self.base_url}, model={self.model}, timeout={timeout}s")
    ...
    def chat_completion(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.debug(f"Chat completion success: {tokens_used} tokens, {duration_ms:.0f}ms")

                return {"content": content, "tokens_used": tokens_used, "model": self.model, "duration_ms": duration_ms}
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            start = time.time()
            result = self.chat_completion(messages=[{"role": "user", "content": "Say 'ok'"}], max_tokens=10)
            latency = (time.time() - start) * 1000

            return {"healthy": True, "model": self.model, "latency_ms": latency, "base_url": self.base_url}
