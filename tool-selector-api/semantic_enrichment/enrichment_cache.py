...
class EnrichmentCache:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        ...
        """
        self.cache_dir = Path(cache_dir or os.getenv("ENRICHMENT_CACHE_DIR", "/app/enrichment_cache"))
        ...
        logger.info(f"EnrichmentCache initialized: {len(self._profiles)} profiles, {len(self._tools)} tools cached")
    ...
    def _save_profiles(self) -> None:
        ...
        try:
            data = {name: profile.model_dump(mode="json") for name, profile in self._profiles.items()}
    ...
    def _save_tools(self) -> None:
        ...
        try:
            data = {tool_id: tool.model_dump(mode="json") for tool_id, tool in self._tools.items()}
    ...
    def needs_profile_update(
        self, server_name: str, current_tools: List[Dict[str, Any]], max_age_days: int = 7
    ) -> bool:
        """
        ...
        if profile.profile_hash != current_hash:
            logger.info(f"Profile for {server_name} has changed: {profile.profile_hash[:8]} -> {current_hash[:8]}")
            return True
    ...
    def needs_tool_enrichment(self, tool: Dict[str, Any], max_age_days: int = 30) -> bool:
        """
    ...
    def get_tools_needing_enrichment(self, tools: List[Dict[str, Any]], max_age_days: int = 30) -> List[Dict[str, Any]]:
        """
