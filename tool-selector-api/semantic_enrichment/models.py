...
class MCPServerProfile(BaseModel):
    ...
    primary_capabilities: List[str] = Field(
        default_factory=list, description="Main capabilities (e.g., ['issue tracking', 'sprint planning'])"
    )
    entity_types: List[str] = Field(
        default_factory=list, description="Entity types handled (e.g., ['issue', 'project', 'sprint'])"
    )
    action_verbs: List[str] = Field(
        default_factory=list, description="Common actions (e.g., ['create', 'update', 'query'])"
    )
    integration_context: str = Field("", description="How this server fits into workflows")
    semantic_keywords: List[str] = Field(default_factory=list, description="Related search terms users might use")
    profile_hash: str = Field("", description="Hash of tool list for change detection")
    last_updated: datetime = Field(default_factory=datetime.utcnow, description="When this profile was last updated")
...
class EnrichedTool(BaseModel):
    ...
    # Enrichment fields
    enhanced_description: str = Field("", description="LLM-generated rich description (200-400 words)")
    action_entities: List[str] = Field(
        default_factory=list, description="Action-entity pairs (e.g., ['create issue', 'update project'])"
    )
    semantic_keywords: List[str] = Field(default_factory=list, description="Search terms from server + tool enrichment")
    use_cases: List[str] = Field(default_factory=list, description="Natural language use case scenarios")
    server_domain: str = Field("", description="Domain from MCP server profile")

    # Metadata
    enrichment_hash: str = Field("", description="Hash of tool content for change detection")
    last_enriched: datetime = Field(default_factory=datetime.utcnow, description="When this tool was last enriched")
    enrichment_model: str = Field("", description="Model used for enrichment (e.g., 'claude-sonnet-4-20250514')")
...
class ServerProfileResult(BaseModel):
    ...
    def from_error(cls, server_name: str, error: Exception) -> "ServerProfileResult":
        """Create a failed result from an exception."""
        return cls(success=False, server_name=server_name, error=str(error), duration_ms=0, tokens_used=0)
