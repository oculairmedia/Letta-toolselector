...
{{
  ...
  "semantic_keywords": ["...", "..."]
    ...
    def profile_server(self, server_name: str, tools: List[Dict[str, Any]], force: bool = False) -> ServerProfileResult:
        """
        ...
        try:
            ...
            prompt = self.PROFILE_PROMPT.format(
                server_name=server_name, tool_count=len(tools), tools_summary=tools_summary
            )

            # Call LLM
            result = self.client.generate_json(prompt=prompt, system_prompt=self.SYSTEM_PROMPT, temperature=0.2)
    ...
    def profile_all_servers(
        self, servers_tools: Dict[str, List[Dict[str, Any]]], force: bool = False
        ...
        logger.info(f"Profiled {success_count}/{len(servers_tools)} servers, {total_tokens} total tokens")

        return results


# Convenience function
def profile_mcp_server(server_name: str, tools: List[Dict[str, Any]], force: bool = False) -> ServerProfileResult:
