...
{{
  ...
  "use_cases": ["...", "..."]
    ...
    def __init__(self, client: Optional[AnthropicClient] = None, cache: Optional[EnrichmentCache] = None):
        """
    ...
    def enrich_tool(
        self, tool: Dict[str, Any], server_profile: Optional[MCPServerProfile] = None, force: bool = False
    ) -> EnrichmentResult:
        """
        ...
        try:
            ...
            # Call LLM
            result = self.client.generate_json(prompt=prompt, system_prompt=self.SYSTEM_PROMPT, temperature=0.3)
    ...
    def enrich_tools(
        self, tools: List[Dict[str, Any]], server_profile: Optional[MCPServerProfile] = None, force: bool = False
        ...
        if not force:
            ...
            logger.info(
                f"Enriching {len(tools_to_process)}/{len(tools)} tools ({len(tools) - len(tools_to_process)} cached)"
            )
        else:
        ...
        logger.info(f"Enriched {success_count}/{len(tools_to_process)} tools, {total_tokens} total tokens")
...
def enrich_tool(
    tool: Dict[str, Any], server_profile: Optional[MCPServerProfile] = None, force: bool = False
) -> EnrichmentResult:
