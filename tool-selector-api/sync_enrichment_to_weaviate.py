...
def ensure_schema_has_enrichment_fields(client) -> bool:
    ...
    if "use_cases" not in existing_props:
        new_props.append(
            Property(name="use_cases", data_type=DataType.TEXT_ARRAY, description="Natural language use case scenarios")
        )
...
def sync_enriched_tools(client, enriched_tools: Dict[str, Dict[str, Any]], dry_run: bool = False) -> Dict[str, int]:
    """
    ...
    for tool_id, enriched in enriched_tools.items():
        ...
        try:
            collection.data.replace(uuid=uuid, properties=merged_props)
            stats["updated"] += 1
...
def main():
    ...
    parser.add_argument("--skip-schema", action="store_true", help="Skip schema update check")
