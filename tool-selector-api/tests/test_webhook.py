...
class TestWebhookCacheOperations:
    ...
    async def test_update_cache_tool_deleted(self, tmp_path):
        ...
        # Start with one tool
        await write_tool_cache([{"id": "tool-123", "name": "existing_tool", "description": "Existing"}])

        # Delete the tool
        await update_cache_for_tool("tool.deleted", {"tool_id": "tool-123", "tool_name": "existing_tool"})
    ...
    async def test_update_cache_tool_updated(self, tmp_path):
        ...
        # Start with one tool
        await write_tool_cache([{"id": "tool-123", "name": "my_tool", "description": "Old description"}])

        # Update the tool
        await update_cache_for_tool(
            "tool.updated", {"tool_id": "tool-123", "tool_name": "my_tool", "description": "New description"}
        )
    ...
    def test_tool_created_payload_structure(self):
        ...
        payload = {
            ...
            "data": {
                ...
                "json_schema": {"type": "object", "properties": {"param1": {"type": "string"}}},
                "tags": ["mcp:my_server"],
            },
        }
