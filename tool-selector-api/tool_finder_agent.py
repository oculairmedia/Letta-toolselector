...
HEADERS = {"Content-Type": "application/json", "Accept": "application/json", "X-BARE-PASSWORD": f"password {API_KEY}"}
...
def make_api_request(method, endpoint, params=None, data=None, stream=False, headers=None, retry_count=0):
    ...
    try:
        ...
        kwargs = {"headers": request_headers, "params": params, "stream": stream}
        if data is not None:
            kwargs["json"] = data
...
def attach_tools_to_agent(agent_id, tool_ids=None, tool_names=None):
    ...
    try:
        ...
        if not agent_id:
            ...
            logging.error(error_msg)
            return {"success": False, "message": error_msg, "processing_results": [], "attachment_results": []}
            ...
            for tool_id in tool_ids:
                try:
                    ...
                    if tool_id not in tool_ids_to_attach:
                        resolved_tool_infos.append({"id": tool_id, "name": tool_name})
                    ...
                    processing_results.append(
                        {"input": tool_id, "type": "id", "success": False, "status": "error", "error": error_message}
                    )
        ...
        if tool_names:
            ...
            try:
                ...
                if isinstance(servers_response, list):
                    ...
                    if "servers" in servers_response:
                        ...
                        "success" in servers_response
                        and servers_response.get("success")
                        and "servers" in servers_response
                    ):
                        ...
                        while server_has_more:
                            try:
                                ...
                                mcp_tools_response = make_api_request(
                                    "GET", f"tools/mcp/servers/{encoded_server}/tools?page={page}&page_size={page_size}"
                                )
                                ...
                                for tool in mcp_tools:
                                    ...
                                    if tool_name and tool_name not in mcp_tool_map:
                                        mcp_tool_map[tool_name] = {"server": server_name, "tool": tool}
                                    elif tool_name in mcp_tool_map:
                                        logging.warning(
                                            f"Duplicate MCP tool name found: '{tool_name}' exists on multiple servers. "
                                            f"Using first found on server '{mcp_tool_map[tool_name]['server']}'."
                                        )

                                if mcp_tools:
                                    logging.info(
                                        f"Found {len(mcp_tools)} tools from MCP server '{server_name}' on page {page}"
                                    )
                                ...
                                if page > 5:
                                    logging.warning(
                                        f"Reached maximum page limit (5) for server {server_name}, stopping pagination"
                                    )
                                    break

                            except Exception as e:
                                logging.error(
                                    f"Error fetching page {page} of tools from MCP server {server_name}: {str(e)}"
                                )
                                server_has_more = False

                        logging.info(
                            f"Fetched a total of {server_tools_count} tools from MCP server '{server_name}' across all pages"
                        )
                ...
                logging.info(
                    f"Built mapping for {len(mcp_tool_map)} unique MCP tools out of {total_mcp_tools} total tools across all servers"
                )
            except Exception as e:
            ...
            for tool_name in tool_names:
                ...
                if tool_name in mcp_tool_map:
                    ...
                    logging.info(
                        f"Found MCP tool '{tool_name}' on server '{mcp_server_name}'. Attempting registration..."
                    )
                ...
                if not found:
                    error_message = (
                        f"Tool name '{tool_name}' not found as an existing Letta tool or a registerable MCP tool."
                    )
                    logging.error(error_message)
                    processing_results.append(
                        {
        ...
        if not resolved_tool_infos:
            ...
            logging.info(
                f"Attempting to attach {len(resolved_tool_infos)} resolved tool(s) to agent {agent_id} ({agent_name})..."
            )
            ...
            for tool in resolved_tool_infos:
                ...
                try:
                    ...
                    if success:
                        ...
                        attachment_results.append(
                            {"tool_id": tool_id, "tool_name": tool_name, "success": False, "error": error_message}
                        )
                except Exception as e:
        ...
        if result.get("success", False):
            ...
            for item in result.get("attachment_results", []):
                if not item.get("success", False):
                    logging.error(
                        f"Failed to attach tool {item.get('tool_name')}: {item.get('error', 'Unknown error')}"
                    )
            return False
...
def get_or_create_memory_block(name, label, value):
    ...
    try:
        ...
        if block_id:
            ...
            logging.error(
                f"❌ Created memory block but couldn't extract ID from response: {json.dumps(response, indent=2)}"
            )
            return None
...
def attach_memory_block_to_agent(
    block_id, agent_id, label=None
):  # Label is not used by the correct API endpoint, but kept for compatibility
    ...
    try:
        ...
        if isinstance(response, dict) and response.get("id") == agent_id:
            ...
            if block_attached:
                ...
                logging.warning(
                    f"⚠️ Attachment API call succeeded for agent {agent_id}, but couldn't verify block {block_id} was attached in response."
                )
                # Consider it a success if the API call didn't fail, but log warning
                return True
        else:
            logging.warning(
                f"Attachment response format unexpected or agent ID mismatch: {json.dumps(response, indent=2)}"
            )
            return False
...
def search_archival_memory(agent_id, query, limit=MAX_SEARCH_RESULTS):
    ...
    try:
        ...
        primary_params = {"search": primary_search_query, "limit": limit}
        ...
        if domain_terms and len(all_passages) < limit:
            for term in domain_terms:
                ...
                domain_params = {"search": term, "limit": limit // 2}
...
def prompt_agent_streaming(agent_id, query):
    ...
    try:
        ...
        message_data = {"messages": [{"role": "user", "content": query}], "stream_steps": True, "stream_tokens": True}

        # Make the streaming request
        response = make_api_request(
            "POST", f"agents/{agent_id}/messages/stream", data=message_data, stream=True, headers=STREAMING_HEADERS
        )
        ...
        if "```json" not in final_response and query.lower().startswith("find the most relevant tools"):
            ...
            for name in tool_names[:5]:  # Limit to 5 tools as per requirements
                tools.append({"name": name.strip(), "description": f"Tool for {name.strip()}"})

            json_response = {"recommended_tools": tools, "query": query}
        ...
        try:
            ...
            endpoints_to_try = [
                # Format 1: New API format from documentation
                {
                    ...
                    "data": {"messages": [{"role": "user", "content": [{"type": "text", "text": query}]}]},
                },
                ...
                {"endpoint": f"agents/{agent_id}/chat", "data": {"message": query}},
            ]
...
def get_existing_passages(agent_id, limit=1000):
    ...
    try:
        ...
        params = {"limit": limit, "ascending": "true"}
...
def upload_tools_to_archival_memory(agent_id, tools, max_workers=10):
    ...
    def _create_single_passage(tool):
        try:
            ...
            synonym_map = {
                ...
                "get": ["retrieve", "fetch"],
                "web": ["internet", "website", "browser", "online", "crawl"],
                "file": ["document", "attachment"],
                "email": ["mail", "message"],
                "github": ["git", "repo", "code"],
                "memory": ["recall", "remember", "store", "archival"],
            ...
            domains = []
            domain_indicators = {
                "web": ["web", "http", "url", "browser", "crawl", "brave"],
                "github": ["github", "git", "repo"],
                "email": ["email", "gmail", "message"],
                "project": ["project", "task", "issue", "plane"],
                "search": ["search", "find", "query"],
                "file": ["file", "document", "pdf"],
                "api": ["api", "endpoint", "request"],
                "database": ["database", "db", "sql"],
                "memory": ["memory", "store", "recall"],
                "agent": ["agent", "assistant", "ai"],
            }
            ...
            if input_schema and isinstance(input_schema, dict):
                ...
                if (
                    "parameters" in input_schema
                    and isinstance(input_schema["parameters"], dict)
                    and "properties" in input_schema["parameters"]
                ):
    ...
    try:
        ...
        logging.info(
            f"Uploading {len(tools_to_upload)} new tools to archival memory for agent {agent_id} using up to {max_workers} workers..."
        )
        ...
        logging.info(
            f"Successfully uploaded/verified {success_count} tools to archival memory ({len(existing_tool_names)} existing, {newly_uploaded_count} new)."
        )
...
def list_all_tools(filter_text=None, page=None, page_size=None):
    ...
    try:
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                logging.error(
                    f"Unexpected response format for native tools endpoint: {type(response)}. Stopping native tool fetch."
                )
                break
            ...
            for server in unique_mcp_servers:
                ...
                while server_has_more:
                    try:
                        ...
                        response = make_api_request(
                            "GET", f"/tools/mcp/servers/{encoded_server}/tools?page={mcp_page}&pageSize={mcp_page_size}"
                        )

                        # Add debug logging to inspect the response structure
                        if DEBUG_MODE:
                            logging.info(
                                f"MCP server {server_name} page {mcp_page} response: {json.dumps(response, indent=2)}"
                            )
                        ...
                        if server_tools:
                            logging.info(
                                f"Found {len(server_tools)} tools from MCP server '{server_name}' on page {mcp_page}"
                            )
                            added_count = 0
    ...
    if filter_text:
        ...
        filtered_tools = [
            ...
            for tool in all_tools
            if (
                (tool.get("name", "").lower().find(filter_lower) != -1)
                or (tool.get("description", "").lower().find(filter_lower) != -1)
                or (tool.get("mcp_server_name", "").lower().find(filter_lower) != -1)
            )
        ]
    ...
    if page is not None and page_size is not None:
        ...
        logging.info(
            f"Paginated tools: page {page}/{total_pages}, showing {len(paginated_tools)} of {total_tools} tools"
        )
...
"""
        ...
        memory_block_id = get_or_create_memory_block(
            name="tool_finder_system_instructions", label="system", value=system_instructions
        )
...
If you can't find exact matches, recommend tools that could be adapted to my needs."""
                            ...
                            for i, tool in enumerate(parsed["recommended_tools"]):
                                ...
                                if tool_issues:
                                    validation_issues.append(
                                        f"Tool #{i + 1} is missing fields: {', '.join(tool_issues)}"
                                    )
            ...
            if target_agent_id:
                ...
                try:
                    ...
                    known_tools = [
                        "brave_web_search",
                        "crawl4ai_basic_crawl",
                        "crawl4ai_advanced_crawl",
                        "crawl4ai_extract_links",
                        "crawl4ai_extract_media",
                        "search_searxng",
                        "analyze_and_search_tool",
                    ...
                    if name_matches:
                        for name in name_matches:
                            ...
                            if (
                                name
                                and name not in direct_tool_names
                                and name.lower() not in common_words
                                and len(name) > 2
                                and (
                                    name in known_tools  # Exact match with known tools
                                    or any(name.startswith(prefix) for prefix in valid_tool_prefixes)
                                    or "_" in name
                                )
                            ):  # Most valid tool names contain underscores
                    ...
                    for name in direct_tool_names:
                        if name in known_tools or any(name.startswith(prefix) for prefix in valid_tool_prefixes):
                    ...
                    try:
                        ...
                        for name in name_matches:
                            if name and len(name) > 2 and not name.lower() in ["tool_name", "tool name", "unknown"]:
                                ...
                                source_pattern = (
                                    rf'"name"\s*:\s*"{re.escape(tool_name)}"[^}}]*?"source_type"\s*:\s*"([^"]+)"'
                                )
                                ...
                                server_pattern = (
                                    rf'"name"\s*:\s*"{re.escape(tool_name)}"[^}}]*?"server"\s*:\s*"([^"]+)"'
                                )
                                ...
                                if not any(t.get("name") == tool_name for t in parsed["recommended_tools"]):
                                    parsed["recommended_tools"].append(
                                        {"name": tool_name, "source_type": source_type, "server": server}
                                    )
                        ...
                        if not parsed["recommended_tools"]:
                            ...
                            for name in backtick_tools:
                                ...
                                if (
                                    tool_name
                                    and len(tool_name) > 2
                                    and not tool_name.lower() in ["tool_name", "tool name", "unknown"]
                                ):
                        ...
                        if (
                            "recommended_tools" in parsed
                            and isinstance(parsed["recommended_tools"], list)
                            and len(parsed["recommended_tools"]) > 0
                        ):
                            ...
                            try:
                                ...
                                for name in final_tool_names:
                                    ...
                                    if (
                                        ...
                                        and not name.strip().lower()
                                        in ["tool_name", "name", "json", "is", "for", "the"]
                                    ):
                                ...
                                for tool_name in valid_tool_names:
                                    ...
                                    if tool_info:
                                        if tool_info.get("source_type") == "mcp" and tool_info.get("server"):
                                            mcp_tools.append({"name": tool_name, "server": tool_info.get("server")})
                                        else:
                                        ...
                                        rec_tool = next(
                                            (
                                                t
                                                for t in parsed.get("recommended_tools", [])
                                                if t.get("name") == tool_name
                                            ),
                                            None,
                                        )
                                        if rec_tool and rec_tool.get("source_type") == "mcp" and rec_tool.get("server"):
                                            mcp_tools.append({"name": tool_name, "server": rec_tool.get("server")})
                                        else:
                                            native_tools.append(tool_name)
                                ...
                                if native_tools:
                                    ...
                                    attachment_result = attach_tools_to_agent(
                                        agent_id=target_agent_id, tool_names=native_tools
                                    )
                                        ...
                                        all_results.append(
                                            {
                                                ...
                                                "message": result.get("message", "Successfully attached.")
                                                if result.get("success", False)
                                                else result.get("error", "Failed to attach tool"),
                                            }
                                        )
                                ...
                                if mcp_tools:
                                    ...
                                    for mcp_tool in mcp_tools:
                                        print(
                                            f"Registering MCP tool {mcp_tool['name']} from server {mcp_tool['server']}..."
                                        )
                                    ...
                                    if registered_mcp_tool_ids:
                                        ...
                                        mcp_attachment_result = attach_tools_to_agent(
                                            agent_id=target_agent_id, tool_ids=registered_mcp_tool_ids
                                        )
                                ...
                                attach_response = {"success": success_count > 0, "attachment_summary": all_results}
