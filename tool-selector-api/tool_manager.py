...
async def process_tools(
    ...
    potential_detach_count = sum(
        1 for tool in mcp_tools if (tool.get("tool_id") or tool.get("id")) not in keep_tool_ids
    )
    ...
    logger.info(
        f"MIN_MCP_TOOLS check: current={len(mcp_tools)}, potential_detach={potential_detach_count}, "
    ...
    if remaining_mcp_tools < MIN_MCP_TOOLS:
        ...
        logger.warning(
            f"Limiting detachments to preserve MIN_MCP_TOOLS={MIN_MCP_TOOLS}. Max allowed: {max_detach_allowed}"
        )
    ...
    if tools_to_detach:
        ...
        detach_tasks = [
            detach_tool(agent_id, str(tool.get("tool_id") or tool.get("id") or "")) for tool in tools_to_detach
        ]
    ...
    for i, result in enumerate(attach_raw_results):
        ...
        if isinstance(result, Exception):
            logger.error(
                f"Exception during parallel attach for tool {tool_name_for_error} ({tool_id_for_error}): {result}"
            )
            failed_attachments.append(
                {"success": False, "tool_id": tool_id_for_error, "name": tool_name_for_error, "error": str(result)}
            )
            ...
            failed_attachments.append(
                {
                    ...
                    "error": result.get("error", "Unknown attachment failure")
...
async def ensure_protected_tools(
    agent_id: str, protected_tool_names: List[str], available_tools: Optional[List[Dict[str, Any]]] = None
) -> Dict[str, Any]:
    """
    ...
    """
    result = {"success": True, "already_attached": [], "newly_attached": [], "failed": [], "errors": []}
        ...
        try:
            ...
            if attach_result.get("success"):
                ...
                result["errors"].append(
                    f"Failed to attach '{tool_name}': {attach_result.get('error', 'Unknown error')}"
                )
                result["success"] = False
        except Exception as e:
            result["failed"].append(tool_name)
...
async def perform_tool_pruning(
    ...
    if _search_tools_func is None:
        return {"success": False, "error": "Search function not configured. Call configure() with search_tools_func."}
    ...
    logger.debug(
        "Requested to keep (all types): %s, Requested newly matched (all types): %s",
        requested_keep_tool_ids,
        requested_newly_matched_tool_ids,
    )
    ...
    try:
        ...
        for tool in current_agent_tools_list:
            ...
            is_never_detach = (
                tool_id in requested_keep_tool_ids
                or tool_id in requested_newly_matched_tool_ids
                or config.should_protect_tool(tool.get("name", ""))
                or tool_id in pinned_tool_ids
            )
        ...
        protected_tool_names = [
            ...
            for tool in core_tools_on_agent
        ...
        logger.info(
            f"Agent {agent_id} has {num_total_attached} total tools: "
        ...
        if num_currently_attached_mcp <= MIN_MCP_TOOLS:
            logger.info(
                f"Agent {agent_id} has {num_currently_attached_mcp} MCP tools, which is at or below the minimum required ({MIN_MCP_TOOLS}). Skipping pruning."
            )
            return {
                "success": True,
        ...
        logger.info(
            f"Found {len(ordered_top_library_tool_info)} unique, potentially relevant tools from library search."
        )
        ...
        logger.info(
            f"Initially keeping newly matched MCP tools (if on agent): {len(final_mcp_tool_ids_to_keep)}. Set: {final_mcp_tool_ids_to_keep}"
        )
        ...
        logger.info(
            f"After adding explicitly requested-to-keep MCP tools (if on agent): {len(final_mcp_tool_ids_to_keep)}. Set: {final_mcp_tool_ids_to_keep}"
        )

        # Handle aggressive pruning when must-keep tools exceed target
        if len(final_mcp_tool_ids_to_keep) >= num_mcp_tools_to_keep:
            logger.info(
                f"Number of must-keep MCP tools ({len(final_mcp_tool_ids_to_keep)}) meets or exceeds target ({num_mcp_tools_to_keep}). Being more aggressive with detachment."
            )
            ...
            if len(final_mcp_tool_ids_to_keep) > aggressive_target:
                ...
                for tool in mcp_tools_on_agent_list:
                    ...
                    if config.should_protect_tool(tool_name):
                        ...
                        logger.info(
                            f"Aggressive pruning: PROTECTING never-detach tool '{tool_name}' (ID: {tool.get('id')})"
                        )
                ...
                for tool_id in requested_newly_matched_tool_ids:
                    if (
                        tool_id in current_mcp_tool_ids
                        and tool_id not in prioritized_keeps
                        and len(prioritized_keeps) < aggressive_target
                    ):
                        prioritized_keeps.add(tool_id)
                ...
                for tool_id, _, tool_type in ordered_top_library_tool_info:
                    if (
                        ...
                        and tool_id in final_mcp_tool_ids_to_keep
                        and tool_id not in prioritized_keeps
                        and len(prioritized_keeps) < aggressive_target
                    ):
                        prioritized_keeps.add(tool_id)
                ...
                logger.info(
                    f"Applied aggressive pruning: reduced to {len(final_mcp_tool_ids_to_keep)} tools (target was {aggressive_target})"
                )
        else:
            ...
            for tool_id, _, tool_type in ordered_top_library_tool_info:
                if (
                    tool_type == "external_mcp"
                    and tool_id in current_mcp_tool_ids
                    and tool_id not in final_mcp_tool_ids_to_keep
                ):
            ...
            logger.info(
                f"After filling remaining slots with other relevant attached MCP tools: {len(final_mcp_tool_ids_to_keep)}. Set: {final_mcp_tool_ids_to_keep}"
            )

        logger.info(
            f"Final set of {len(final_mcp_tool_ids_to_keep)} MCP tool IDs decided to be kept on agent: {final_mcp_tool_ids_to_keep}"
        )
        ...
        if mcp_tools_to_detach_ids:
            ...
            for i, result in enumerate(detach_results):
                ...
                if isinstance(result, Exception):
                    logger.error(
                        f"Exception during detach for MCP tool {tool_name_detached} ({tool_id_detached}): {result}"
                    )
                    failed_detachments_info.append(
                        {"tool_id": tool_id_detached, "name": tool_name_detached, "error": str(result)}
                    )
                elif isinstance(result, dict) and result.get("success"):
                    successful_detachments_info.append({"tool_id": tool_id_detached, "name": tool_name_detached})
                else:
                    error_msg = (
                        result.get("error", "Unknown detachment failure")
                        if isinstance(result, dict)
                        else "Unexpected result type"
                    )
                    logger.warning(
                        f"Failed detach result for MCP tool {tool_name_detached} ({tool_id_detached}): {error_msg}"
                    )
                    failed_detachments_info.append(
                        {"tool_id": tool_id_detached, "name": tool_name_detached, "error": error_msg}
                    )
            logger.info(
                f"Successfully detached {len(successful_detachments_info)} MCP tools, {len(failed_detachments_info)} failed."
            )
        else:
