...
INTENT_SYNONYMS: Dict[OperationIntent, Set[str]] = {
    ...
    OperationIntent.LIST: {"list", "enumerate", "browse", "catalog", "inventory", "index", "all"},
    ...
    OperationIntent.ATTACH: {"attach", "connect", "link", "bind", "associate", "join"},
    OperationIntent.DETACH: {"detach", "disconnect", "unlink", "unbind", "disassociate", "remove"},
    OperationIntent.SEND: {"send", "transmit", "deliver", "dispatch", "forward", "message", "notify"},
    OperationIntent.CONFIGURE: {"configure", "setup", "settings", "preferences", "options", "customize"},
}
...
class ToolSchemaAnalyzer:
    ...
    def analyze_tool(self, tool: Dict[str, Any]) -> ToolCapabilities:
        ...
        caps = ToolCapabilities(tool_name=name, tool_id=tool.get("id", tool.get("tool_id", "")))
...
class UniversalQueryExpander:
    ...
    def __init__(self, tool_cache_path: Optional[str] = None):
        ...
        self._intent_to_tools: Dict[OperationIntent, Set[str]] = {intent: set() for intent in OperationIntent}
    ...
    def _analyze_tools(self, tools: List[Dict[str, Any]]):
        ...
        multifunc_tools = [
            name for name, caps in self._tool_capabilities.items() if caps.multifunctional_confidence > 0.3
        ]
    ...
    def expand_query(self, query: str) -> ExpandedQuery:
        ...
        if len(added_keywords) > MAX_EXPANSION_KEYWORDS:
            ...
            added_keywords = priority_keywords | set(
                list(other_keywords)[: MAX_EXPANSION_KEYWORDS - len(priority_keywords)]
            )
