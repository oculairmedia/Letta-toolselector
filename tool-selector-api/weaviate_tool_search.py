            ...
            try:
                if is_qwen3_format_enabled() and hasattr(provider, "get_embeddings_with_instructions"):
                    result = await provider.get_embeddings_with_instructions(
                        [text], task_description=get_search_instruction()
                    )
...
def get_embedding_for_text(text: str, enhance_prompt: bool = True) -> list:
    ...
    try:
        ...
        if enhance_prompt and is_qwen3_format_enabled():
            embedding_text = get_detailed_instruct(get_search_instruction(), format_query_for_qwen3(text))
        elif enhance_prompt:
...
def get_tool_embedding_by_id(tool_id: str) -> List[float]:
    ...
    try:
        ...
        response = collection.query.fetch_objects(
            filters=weaviate.classes.query.Filter.by_id().equal(tool_id), limit=1, include_vector=True
        )
        ...
        if response.objects and len(response.objects) > 0:
            ...
            if hasattr(tool_obj, "vector") and tool_obj.vector is not None and len(tool_obj.vector) > 1:
                ...
                print(
                    f"⚠️ No valid vector attribute for tool {tool_id} via fetch_objects (vector: {tool_obj.vector if hasattr(tool_obj, 'vector') else 'N/A'})."
                )
        else:
        ...
        if (
            hasattr(gql_response, "get")
            and gql_response.get
            and "Tool" in gql_response.get
            and len(gql_response.get["Tool"]) > 0
            and "_additional" in gql_response.get["Tool"][0]
            and "vector" in gql_response.get["Tool"][0]["_additional"]
        ):
            ...
            if isinstance(vector, list) and all(isinstance(x, (int, float)) for x in vector) and len(vector) > 1:
                ...
                print(
                    f"⚠️ GraphQL fallback for tool {tool_id} returned invalid vector (type: {type(vector)}, len: {len(vector) if isinstance(vector, list) else 'N/A'})."
                )
        else:
